...
function openEdit(row: Record<string, unknown>) {
  Object.assign(form, {
    ...
    water_rate: Number(row.water_rate ?? 0.085),
  });
  ...
  <div :class="['page-card', { 'audit-page-card': resource === 'audit-logs', 'bet-records-page-card': resource === 'bet-records' }]">
    ...
    <div v-if="resource === 'bet-records'" class="bet-view-switch">
      <el-radio-group :model-value="betViewMode" @change="changeBetView($event as BetViewMode)">
        ...
        <el-radio-button value="play">汇总表格</el-radio-button>
      </el-radio-group>
      <span v-if="betViewMode === 'play'">共读取 {{ aggregationSourceCount }} 个标准投注项，按总下注金额排序</span>
    </div>
    <section v-if="resource === 'bet-records' && betViewMode !== 'records'" class="aggregation-panel">
      ...
      <el-alert v-if="aggregationUnmappedCount > 0" :title="`${aggregationUnmappedCount} 个历史投注项缺少标准玩法，已归入其他。`" type="warning" :closable="false" show-icon />
    ...
    /><el-drawer
      ...
      ><el-form label-width="96"
        ...
        ><el-form-item v-if="resource === 'agent-center'" label="明水比例"
          ><el-input-number v-model="form.water_rate" :min="0" :max="1" :precision="4" :step="0.001" controls-position="right" style="width: 100%" /><div class="lottery-tip">明水 = 占成金额 × 此比例；默认 0.085（8.5%）。系统不再区分离线反水、暗水或其他明水。</div></el-form-item
        ><el-form-item
...
.aggregation-number-list { max-height: 150px; overflow-y: auto; padding: 2px 0; }
.aggregation-number-item { display: flex; align-items: baseline; justify-content: space-between; gap: 10px; padding: 2px 0; border-bottom: 1px dotted #edf0f5; }
