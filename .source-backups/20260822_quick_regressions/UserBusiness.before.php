...
final class UserBusiness
{
    ...
    private function lineForLottery(array $line,string $lottery): array
    {
    ...
    public function quickPreview(Request $request): \think\response\Json
    {
        ...
        foreach ($lines as &$line) if ($line['status']==='success') {
            ...
            foreach($lineLotteries as $lineLottery){$this->assertLotteryPermission($s,$lineLottery);if(!$this->lineOdds($s,$lineLottery,$line))$oddsReady=false;}
            if (!$oddsReady) { $line['status']='failed'; $line['reason']='当前玩法无法唯一匹配赔率'; $line['amount']='0.00'; $line['count']=0; continue; }
    ...
    public function quickPlace(Request $request): \think\response\Json
    {
        ...
        try {
            foreach ($lines as $line) foreach ($this->lotteriesForLine($line,$lottery) as $lineLottery) {
                ...
                $splitLine=$this->lineForLottery($line,$lineLottery);
        ...
        try {
            $recordIds=Db::transaction(function () use ($s,$text,$formattedText,$groups,$amount,$now): array {
                ...
                foreach ($groups as $lineLottery=>$group) {
                    ...
                    $recordId=(int)Db::name('bet_records')->insertGetId(['tenant_id'=>$s['tenant_id'],'site_id'=>$s['site_id'],'user_id'=>$s['user_id'],'issue_no'=>$issueNo,'source_text'=>$text,'formatted_text'=>$formattedText,'bet_count'=>$recordCount,'amount'=>$recordAmount,'win_amount'=>0,'status'=>'pending','sealed'=>0,'placed_at'=>$now,'created_at'=>$now]);
                    $recordIds[]=$recordId;
