...
final class AgentLedger
{
    ...
    public function issues(Request $request): \think\response\Json
    {
        ...
        $rows=Db::name('lottery_histories')->where('lottery_id',(int)$lottery['id'])->where('draw_day','>=',date('Y-m-01'))->where('draw_day','<=',date('Y-m-t'))->field('code AS issue_no,draw_day AS date')->order('draw_day desc')->order('code desc')->select()->toArray();
    ...
    {
        if($issues===[])return[];$query=Db::name('bet_details')->alias('d')->join('bet_records r','r.id=d.bet_record_id')->join('site_users u','u.id=d.user_id')->leftJoin('user_stop_drops s','s.bet_detail_id=d.id')->where('d.site_id',(int)$session['site_id'])->whereIn('d.issue_no',$issues)->where('s.lottery',$lottery)->where('r.status','<>','refunded');OrganizationHierarchy::applyUserScope($query,$session,'d.user_id');$rows=$query->field('d.id,d.user_id,u.username,d.issue_no,d.category,d.number_text,d.amount,d.win_amount,d.rebate')->select()->toArray();if($rows===[])return[];
        ...
        foreach($rows as &$row){$amount=(float)$row['amount'];$win=(float)$row['win_amount'];$rebate=(float)$row['rebate'];$memberProfit=$win+$rebate-$amount;$intercepted=(float)($map[(int)$row['id']]['intercepted']??0);$ratio=$amount>0?min(1,$intercepted/$amount):0;$numbers=preg_split('/[\s,，]+/u',trim((string)$row['number_text']),-1,PREG_SPLIT_NO_EMPTY)?:[];$row['bet_count']=max(1,count($numbers));$row['intercepted']=$intercepted;$row['requested_share']=(float)($map[(int)$row['id']]['requested']??0);$row['share_rate']=(float)($map[(int)$row['id']]['share_rate']??0);$row['house_profit']=-$memberProfit;$row['share_profit']=(-$memberProfit)*$ratio;}unset($row);return$rows;
    }

    private function contribution(array $details): array
    {
        $groups=[];foreach($details as $row){$key=(int)$row['user_id'];if(!isset($groups[$key]))$groups[$key]=['member'=>(string)$row['username'],'share_amount'=>0.0,'share_total_amount'=>0.0,'share_total_profit'=>0.0,'percentage_share_profit'=>0.0,'actual_share_profit'=>0.0,'share_percentage'=>0.0,'contribution'=>0.0];$groups[$key]['share_amount']+=(float)$row['intercepted'];$groups[$key]['share_total_amount']+=(float)$row['amount'];$groups[$key]['share_total_profit']+=(float)$row['house_profit'];$groups[$key]['percentage_share_profit']+=(float)$row['house_profit']*(float)$row['share_rate']/100;$groups[$key]['actual_share_profit']+=(float)$row['share_profit'];}
        $totalActual=array_sum(array_column($groups,'actual_share_profit'));foreach($groups as &$row){$row['share_percentage']=$row['share_total_amount']>0?$row['share_amount']/$row['share_total_amount']*100:0;$row['contribution']=abs($totalActual)>0?abs($row['actual_share_profit'])/abs($totalActual)*100:0;foreach(['share_amount','share_total_amount','share_total_profit','percentage_share_profit','actual_share_profit']as$key)$row[$key]=$this->number((float)$row[$key]);foreach(['share_percentage','contribution']as$key)$row[$key]=$this->number((float)$row[$key]).'%';}unset($row);return array_values($groups);
    }
    ...
    private function ledgerRows(array $details,string $view,array $session): array
    {
        ...
        foreach($details as $row){$head=$monthly?(string)$row['issue_no']:($path?$agentName:(string)$row['username']);$key=$head.'|'.(string)$row['category'];if(!isset($groups[$key]))$groups[$key]=['label'=>$head,'category'=>(string)($row['category']?:'未分类'),'bet_count'=>0,'amount'=>0.0,'rebate'=>0.0,'win_amount'=>0.0,'profit'=>0.0];$groups[$key]['bet_count']+=(int)$row['bet_count'];$groups[$key]['amount']+=(float)$row['amount'];$groups[$key]['rebate']+=(float)$row['rebate'];$groups[$key]['win_amount']+=(float)$row['win_amount'];$groups[$key]['profit']+=(float)$row['amount']-(float)$row['rebate']-(float)$row['win_amount'];}
        foreach($groups as &$row)foreach(['amount','rebate','win_amount','profit']as$key)$row[$key]=$this->number((float)$row[$key]);unset($row);return array_values($groups);
    }
