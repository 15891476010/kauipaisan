...
final class UserBusiness
{
    ...
    /**
     * Older compact-play records keep `000` as an internal settlement
     * placeholder because the wager is a selection set (for example
     * `15862 组六五码`), not one literal three-digit number.  The placeholder
     * is useful to the settlement engine but is misleading in the user's
     * detail table.  Recover a display-only selection from the stored source;
     * never rewrite the persisted number used by settlement.
     */
    private function detailDisplayNumberText(array $row, string $source): string
    {
        $stored=trim((string)($row['number_text']??''));
        if ($stored!=='' && $stored!=='000') return $stored;
        $source=trim($source);
        if ($source==='') return $stored!==''?$stored:'-';
        $normalized=(new \app\service\QuickEntryRules())->normalize($source);
        $normalized=preg_replace('/\s+/u',' ',trim($normalized))??trim($normalized);

        // Catalog/group selections: retain the selected digit set (including
        // two-digit selections) rather than showing the internal 000 marker.
        // QuickEntryRules normalizes Chinese count words (for example
        // “五码”) to their Arabic form (“5码”), so accept both spellings.
        if (preg_match('/(?<!\d)(\d{1,10})\s*(?=(?:组三|组六|复式)(?:[一二两三四五六七八九1-9]\s*)?码)/u',$normalized,$match)===1) {
            return (string)$match[1];
        }
        // Some older rows omit the “码” suffix in their settlement text.
        if (preg_match('/(?<!\d)(\d{1,10})\s*(?=(?:组三|组六|复式)(?:\s|$))/u',$normalized,$match)===1) {
            return (string)$match[1];
        }
        // Keep a compact representation for胆拖 selections.
        if (preg_match('/(胆\s*[0-9]{1,2}\s*拖\s*[0-9]{2,9})/u',$normalized,$match)===1) {
            return preg_replace('/\s+/u','',(string)$match[1])??(string)$match[1];
        }
        // Position selections are already expanded for new rows; this fallback
        // keeps old placeholder rows understandable without changing amounts.
        if (preg_match('/((?:百|十|个)\s*[0-9]+(?:\s+(?:百|十|个)\s*[0-9]+){1,2})/u',$normalized,$match)===1) {
            return preg_replace('/\s+/u',' ',trim((string)$match[1]))??(string)$match[1];
        }
        // Outcome/package plays have no literal number. Show the play name
        // instead of the misleading 000 marker.
        if (preg_match('/(和值\s*(?:2[0-7]|1\d|\d)|跨度\s*\d|和[大小单双]|大小单双|豹子全包|对子全包|双飞|独胆)/u',$normalized,$match)===1) {
            return preg_replace('/\s+/u','',(string)$match[1])??(string)$match[1];
        }
        $play=trim((string)($row['play_type']??''));
        return $play!==''?$play:$stored;
    }
    ...
    public function betDetails(Request $request): \think\response\Json
    {
        ...
        $number=trim((string)$request->param('number',''));if($number!=='')$query->where(function($nested)use($number):void{$nested->whereLike('d.number_text','%'.$number.'%')->whereOrLike('d.source_text','%'.$number.'%')->whereOrLike('r.source_text','%'.$number.'%');});
        ...
        $detailRows=$query->field($fields)->order('d.placed_at',$sort)->order('d.id',$sort)->select()->toArray(); $expanded=[];
        foreach($detailRows as $row){
            $source=(string)($row['detail_source']??'');if($source==='')$source=(string)($row['record_source']??'');$storedTokens=$this->detailNumberTokens($row['number_text']??'');$displayText=$this->detailDisplayNumberText($row,$source);$tokens=$storedTokens;
            if(($tokens===[]||($tokens===['000']&&$displayText!==''&&$displayText!=='000'))&&$displayText!=='')$tokens=$this->detailNumberTokens($displayText);
            if($tokens===[])$tokens=['-'];if($sort==='desc')$tokens=array_reverse($tokens);$count=count($tokens);
            $amounts=$this->splitDetailMoney((float)$row['amount'],$count);$wins=$this->splitDetailMoney((float)$row['win_amount'],$count);$rebates=$this->splitDetailMoney((float)$row['rebate'],$count);$orderNo=$this->detailOrderNumber($row);
            foreach($tokens as $index=>$token){$amount=(float)$amounts[$index];$win=(float)$wins[$index];$rebate=(float)$rebates[$index];$expanded[]=['id'=>(int)$row['id'],'row_key'=>(int)$row['id'].'-'.$index,'bet_record_id'=>(int)($row['bet_record_id']??0),'submission_id'=>(int)($row['submission_id']??0)?:null,'order_no'=>$orderNo,'issue_no'=>(string)$row['issue_no'],'number_text'=>$token,'stored_number_text'=>$storedTokens[$index]??($storedTokens[0]??''),'category'=>(string)($row['category']??''),'play_type'=>(string)($row['play_type']??''),'play_label'=>$this->detailPlayLabel($row['play_type']??'',$row['category']??''),'lottery'=>(string)($row['lottery']??''),'amount'=>$amounts[$index],'odds'=>$row['odds']===null?'-':rtrim(rtrim(number_format((float)$row['odds'],3,'.',''),'0'),'.'),'win_amount'=>$wins[$index],'rebate'=>$rebates[$index],'offline_rebate'=>'0','profit'=>$this->detailMoney($win-$amount+$rebate),'status'=>(string)($row['status']??$row['record_status']??'pending'),'placed_at'=>(string)$row['placed_at'],'source_text'=>$source];}
        }
