...
final class BetSettlement
{
    ...
    private function matches(string $number, string $draw, string $source): bool
    {
        ...
        if (str_contains($source, '双飞')) {
            $pair=str_split(substr($number,-2));
    ...
    private function matchesCompactExpression(string $number,string $draw,string $source): ?bool
    {
        ...
        if(preg_match('/^胆(\d+)拖(\d+)$/u',$compact,$match)){
            ...
            return array_diff($dan,$drawDigits)===[]&&array_diff($drawUnique,$allowed)===[];
        }
        if($compact==='豹子')return count($drawUnique)===1;
        if($compact==='对子')return count($drawUnique)===2;
        ...
        if(str_contains($source,'定位')||preg_match('/^[0-9]+D$/i',$compact)){
            $positionRules=[];
        ...
        if(str_contains($source,'独胆')||preg_match('/^\d胆$/u',$compact)){
