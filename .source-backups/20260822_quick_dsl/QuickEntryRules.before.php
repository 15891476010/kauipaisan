...
final class QuickEntryRules
{
    ...
    private const PATTERNS = [
        ...
        'amount_after_play' => '/(直|单|组三|组六|组)\s*(\d+(?:\.\d+)?)\s*(倍|元|米|块|角|毛)?(?=\s*(?:共|合计|总计|福|体|$))/u',
        'trailing_multiplier' => '/(\d+(?:\.\d+)?)\s*倍\s*$/u',
        'trailing_short_multiplier' => '/(?<!\d)(\d{1,2})\s*(直|单|组)\s*$/u',
        'digit_set_head' => '/^\s*(?:福体|福|体)?\s*([0-9]{4,10})\s*(?:福体|福|体)?\s*(.+)$/u',
        ...
        'size_parity_bet' => '/^\s*(?:福体|福|体)?\s*(和大|和小|和单|和双|大|小|单|双)\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛)?\s*$/u',
        ...
        'catalog_digit_set_bet' => '/^\s*(?:福体|福|体)?\s*([0-9]{1,10})\s*(组三赖|组六赖|组三|组六|复式)\s*([一二三四2-9])码\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛)?\s*$/u',
        'multi_play_shared_amount' => '/((?:(?:组三|组六|复式)\s*){2,3})(?:各|每)\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛|倍)?/u',
        'multi_play_amount' => '/(组三|组六|复式)(?:各|每)\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛|倍)?/u',
    ...
    public function normalize(string $text): string
    {
        ...
        $text = preg_replace('/^\s*(\d{3})\s+(\d+(?:\.\d+)?)\s*$/u', '$1直$2元', $text) ?? $text;
