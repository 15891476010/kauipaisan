...
import {
  getAgentMembers,
  type AgentMember,
  type AgentMemberList,
} from "../../api/user";

const emptyResult: AgentMemberList = {
  list: [],
  total: 0,
  page: 1,
  page_size: 40,
...
export function SubordinatesPage({ agentName }: { agentName: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getAgentMembers({
        ...applied,
        page,
        page_size: 40,
      });
  ...
  return (
    <section className="subordinate-page">
      <div className="subordinate-location">
        <div className="subordinate-path">
          <strong>位置</strong>
          <DoubleRightOutlined />
          <span>下级管理</span>
          <DoubleRightOutlined />
      ...
      <div className="subordinate-content">
        <div className="credit-summary">
          <strong>{agentName}(代理)</strong>
          <span>总信用额度：</span>
          <b>{result.total_credit}</b>
          <i>（0）</i>
          <span>可分配信用额度：</span>
          <b className="available">{result.available_credit}</b>
          <i>（0）</i>
          <span>已分配信用额度：</span>
          <b className="allocated">{result.allocated_credit}</b>
          <i>（0）</i>
        </div>
        <form
        ...
        >
          <div className="member-filter-field">
          ...
          <div className="member-filter-field">
            <label htmlFor="member-code">代号：</label>
            <input
              id="member-code"
              maxLength={20}
              value={filters.code}
              onChange={(event) =>
                setFilters({ ...filters, code: event.target.value })
              }
              placeholder="搜索代号"
            />
          </div>
          <div className="member-filter-field member-status-field">
          ...
          <button className="member-search" type="submit">
            <SearchOutlined />
            搜索
          </button>
          <button className="member-create" type="button" onClick={openCreate}>
            <PlusOutlined />
            新增会员
          </button>
        </form>
                          ...
                          modal.info({
                            ...
                            content: (
                              <div className="member-detail">
                                ...
                                <p>代号：{member.display_name}</p>
                                <p>信用额度：{member.credit_balance}</p>
                                <p>可用余额：{member.available_balance}</p>
                              </div>
                            ),
                            okText: "关闭",
                          })
