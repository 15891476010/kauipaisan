...
final class UserBusiness
{
    ...
    public function quickPreview(Request $request): \think\response\Json
    {
        ...
        foreach ($lines as &$line) if ($line['status']==='success') {
            if (!$this->lineOdds($s,$lottery,$line)) { $line['status']='failed'; $line['reason']='当前玩法无法唯一匹配赔率'; $line['amount']='0.00'; $line['count']=0; continue; }
            $count+=(int)$line['count']; $amount+=(float)$line['amount'];
        }
        unset($line);
    ...
    public function quickPlace(Request $request): \think\response\Json
    {
        ...
        $lotteryId=(int)Db::name('lotteries')->where('name',$lottery)->where('status',1)->whereNull('deleted_at')->value('id');
        $nextHistory=$lotteryId>0 ? Db::name('lottery_histories')->where('lottery_id',$lotteryId)->whereNotNull('next_code')->where('next_code','<>','')->order('open_time desc')->order('id desc')->find() : null;
        $pendingHistory=$lotteryId>0 ? Db::name('lottery_histories')->where('lottery_id',$lotteryId)->where(function($q): void { $q->whereNull('numbers')->whereOr('numbers',''); })->where('open_time','>=',date('Y-m-d H:i:s'))->order('open_time asc')->order('id asc')->find() : null;
        ...
        $lines=$this->quickLines($text,$lottery,(int)$s['tenant_id']); $valid=array_values(array_filter($lines,static fn(array $line): bool=>$line['status']==='success')); if (!$valid) return $this->reply(null,'没有可下注的有效内容',422);
        $now=date('Y-m-d H:i:s'); $amount=0.0; $count=0; $lineRules=[]; foreach ($valid as $index=>$line) { try{$rule=$this->applyLineLimits($s,$lottery,$line);}catch(\InvalidArgumentException $e){return $this->reply(null,$e->getMessage(),422);} $lineRules[$index]=$rule; $amount+=(float)$rule['actual']; $count+=(int)$line['count']; }
        ...
        try {
            $recordId=(int)Db::transaction(function () use ($s,$text,$formattedText,$lottery,$lotteryId,$valid,$lineRules,$amount,$count,$now,$issueNo): int {
                ...
                $recordId=(int)Db::name('bet_records')->insertGetId(['tenant_id'=>$s['tenant_id'],'site_id'=>$s['site_id'],'user_id'=>$s['user_id'],'issue_no'=>$issueNo,'source_text'=>$text,'formatted_text'=>$formattedText,'bet_count'=>$count,'amount'=>$amount,'win_amount'=>0,'status'=>'pending','sealed'=>0,'placed_at'=>$now,'created_at'=>$now]);
                foreach ($valid as $index=>$line) {
                    $rule=$lineRules[$index];
                ...
                CreditLedger::userBet($s,(int)$s['user_id'],$amount,$before,$recordId,$issueNo);
                return $recordId;
            });
        } catch (\Throwable $e) {
        ...
        return $this->reply(['record_id'=>$recordId,'count'=>$count,'amount'=>number_format($amount,2,'.',''),'formatted_text'=>$formattedText],'下注提交成功');
    }
