...
final class QuickEntryParser
{
    ...
    private function parseExplicitPlayBets(string $rawOriginal, string $working, string $category, int $lineId, ?float $declaredTotal, float $unitStake): ?array
    {
        ...
        if ($combinedMatched === 0 && preg_match_all($this->rules->pattern('amount_after_play'), $working, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                $unitAmount=$unit===''||$unit==='倍'?(float)$match[2]*$unitStake:$this->rules->amountWithUnit((float)$match[2],$unit,$unitStake);
        ...
        foreach ($specs as $spec) {
            $groups = $spec['play'] === '组' ? $this->groupNumbersByPlay($numbers) : ['直' => $numbers];
    ...
    private function parseGroupedDigitSet(string $rawOriginal, string $working, string $category, int $lineId, ?float $declaredTotal, float $unitStake): ?array
    {
        ...
        foreach ($matches as $match) {
            ...
            if ($identity === null) {
                if ($play !== '组六' || count($digits) !== 3) return [$this->failure($lineId,$rawOriginal,'所选数字数量与玩法不一致')];
                $playType='组六'; $numberText=implode('',$digits);
            } else {
    ...
    private function parseDragBets(string $rawOriginal,string $raw,string $category,int $lineId,float $unitStake): ?array
    {
        ...
        $specs=[];
        if($double){
            if(preg_match_all($this->rules->pattern('multi_play_amount'),$raw,$amounts,PREG_SET_ORDER))foreach($amounts as $amount)if($amount[1]==='组六')$specs[]=['play'=>'组六','amount'=>$this->rules->amountWithUnit((float)$amount[2],(string)($amount[3]??''),$unitStake)];
            if($specs===[]&&preg_match($this->rules->pattern('per_unit_amount'),$raw,$amount))$specs[]=['play'=>'组六','amount'=>$this->rules->amountWithUnit((float)$amount[1],(string)($amount[2]??''),$unitStake)];
        }else{
            if(preg_match($this->rules->pattern('multi_play_shared_amount'),$raw,$shared)){preg_match_all($this->rules->pattern('multi_play_name'),$shared[1],$plays);foreach(array_unique($plays[0]??[])as$play)if(in_array($play,['组三','组六'],true))$specs[]=['play'=>$play,'amount'=>$this->rules->amountWithUnit((float)$shared[2],(string)($shared[3]??''),$unitStake)];}
            elseif(preg_match_all($this->rules->pattern('multi_play_amount'),$raw,$amounts,PREG_SET_ORDER))foreach($amounts as $amount)if(in_array($amount[1],['组三','组六'],true))$specs[]=['play'=>$amount[1],'amount'=>$this->rules->amountWithUnit((float)$amount[2],(string)($amount[3]??''),$unitStake)];
        }
    ...
    private function parseCatalogBet(string $rawOriginal, string $raw, string $category, int $lineId, float $unitStake): ?array
    {
        ...
        return [
            ...
            'number_text'=>'000', 'category'=>$category, 'amount'=>number_format($amount,2,'.',''),
            'count'=>$category==='福体'?2:1, 'play_type'=>$playType, 'settlement_text'=>$settlementText,
        ];
    }
