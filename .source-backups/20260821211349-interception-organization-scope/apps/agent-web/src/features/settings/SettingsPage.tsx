...
const emptyProfile: AgentSettingProfile = { username: '', display_name: '', remark: '', share_limit: '0', follow_share: 0, total_credit: '0', allocated_credit: '0', available_credit: '0' };
  ...
  return <section className="agent-settings-page">
    ...
    {loading ? <div className="agent-settings-loading"><Spin /></div> : <>
      <div className="agent-settings-profile">
        ...
        <div className="agent-settings-profile-item follow"><Tooltip title="开启后将跟随总后台吃货；总后台该号码已吃满时，本代理停止继续吃货。"><span><QuestionCircleOutlined /> 随盘占成</span></Tooltip><Switch aria-label="是否跟随总后台吃货" checked={followShare} onChange={setFollowShare} /></div>
      </div>
      ...
      <div className="agent-settings-actions"><span>拦货金额设置100 = 每个号码可以拦到100</span><button type="button" disabled={saving} onClick={submit}>{saving?'保存中':'提 交'}</button></div>
      <div className="agent-settings-table-wrap"><table className="agent-settings-table"><thead><tr><th>类别</th><th>最小下注</th><th>赔率上限</th><th>拦货金额(同 □)</th><th>单注上限</th><th>单项上限</th></tr></thead><tbody>{groups.flatMap((group) => {
        const categoryRow = group.rows.length > 1 ? <tr className="settings-category-row" key={`category-${group.name}`}><td>{group.name}</td><td /><td /><td>（同 <input type="checkbox" checked={Boolean(synced[group.name])} onChange={(event) => setSynced((current) => ({...current,[group.name]:event.target.checked}))} />）</td><td /><td /></tr> : null;
        return [categoryRow,...group.rows.map((row) => <tr key={row.id}><td>{row.name}</td><td>{showNumber(row.min_bet)}</td><td className="settings-odds">{showNumber(row.odds_limit)}</td><td><input className="settings-amount-input" inputMode="decimal" value={amounts[String(row.id)] ?? '0'} onChange={(event) => changeAmount(row,event.target.value)} /></td><td>{showNumber(row.single_bet_limit)}</td><td>{showNumber(row.single_item_limit)}</td></tr>)];
      })}</tbody></table></div>
    </>}
  </section>;
}
