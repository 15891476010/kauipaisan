...
final class AgentInterception
{
    ...
    public function plate(Request $request): \think\response\Json
    {
        ...
        $node=OrganizationHierarchy::nodeForSession($session);$organizationSettings=$node?json_decode((string)($node['settings']??''),true):[];$organizationSettings=is_array($organizationSettings)?$organizationSettings:[];
        $amounts=$organizationSettings['agent_interception_amounts_'.$lotteryId]??json_decode((string)Db::name('settings')->where('site_id', $siteId)->where('key', 'agent_interception_amounts_'.$lotteryId)->value('value'), true);
        if (!is_array($amounts)) $amounts = [];
        $usageQuery=Db::name('agent_interceptions')->where('site_id',$siteId)->where('lottery_id',$lotteryId)->where('issue_no',$issue)->whereNull('released_at');
        OrganizationHierarchy::applyUserScope($usageQuery,$session,'user_id');
        $usageRows=$usageQuery->field('lottery_odds_id,number_key,SUM(intercepted_amount) AS used_amount')->group('lottery_odds_id,number_key')->order('number_key asc')->select()->toArray();
        $usage = []; foreach ($usageRows as $row) $usage[(int)$row['lottery_odds_id']][] = ['number' => (string)$row['number_key'], 'used' => $this->number((float)$row['used_amount'])];
        ...
        foreach ($odds as $row) {
            ...
            foreach ($usage[$id] ?? [] as $item) $used += (float)$item['used'];
            $category = (string)($row['category'] ?: '其他');
            $groups[$category][] = ['odds_id' => $id, 'name' => (string)$row['name'], 'odds' => $this->number((float)$row['odds']), 'limit' => $this->number($limit), 'used' => $this->number($used), 'remaining' => $this->number(max(0, $limit - $used)), 'numbers' => $usage[$id] ?? []];
        }
        $list = []; foreach ($groups as $category => $items) $list[] = ['category' => $category, 'items' => $items];
        return $this->reply(['issue_no' => $issue, 'groups' => $list]);
    }
