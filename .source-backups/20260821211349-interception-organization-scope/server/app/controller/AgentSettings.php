...
final class AgentSettings
{
    ...
    private function settingKey(int $lotteryId): string { return 'agent_interception_amounts_'.$lotteryId; }
    ...
    private function amounts(array $session, int $lotteryId): array
    {
        $settings=$this->organizationSettings($session);$key=$this->settingKey($lotteryId);
    ...
    private function odds(int $lotteryId, array $amounts): array
    {
        $rows=Db::name('lottery_odds')->where('lottery_id',$lotteryId)->where('status',1)->whereNull('deleted_at')->order('sort asc')->order('id asc')->select()->toArray();
        $direct=Db::name('lottery_odds_categories')->where('lottery_id',$lotteryId)->where('is_playable',1)->where('status',1)->whereNull('deleted_at')->order('sort asc')->order('id asc')->select()->toArray();
    ...
    private function profile(array $session): array
    {
        ...
        $node=OrganizationHierarchy::nodeForSession($session);$settings=$this->organizationSettings($session);$siteId=(int)$session['site_id'];
        $follow=array_key_exists(self::FOLLOW_SETTING_KEY,$settings)?((int)$settings[self::FOLLOW_SETTING_KEY]===1?1:0):((int)Db::name('settings')->where('site_id',$siteId)->where('key',self::FOLLOW_SETTING_KEY)->value('value')===1?1:0);
        ...
        return array_merge(['username'=>(string)($row['username']??($session['username']??'')),'display_name'=>(string)($row['display_name']??''),'remark'=>(string)($node['name']??''),'share_limit'=>number_format($shareLimit,4,'.',''),'follow_share'=>$follow],$credit);
    }
    ...
    public function index(Request $request): \think\response\Json
    {
        ...
        $amounts=$this->amounts($session,(int)$lottery['id']);
        return $this->reply(['profile'=>$this->profile($session),'lottery'=>$lottery,'odds'=>$this->odds((int)$lottery['id'],$amounts)]);
    }

    public function save(Request $request): \think\response\Json
    {
        $session=$this->session($request); $data=$request->put(); $lottery=$this->lottery($session,trim((string)($data['lottery']??'')));
        $input=$data['amounts']??null; if(!is_array($input)) throw new \InvalidArgumentException('拦货金额格式不正确');
        $valid=array_map('intval',Db::name('lottery_odds')->where('lottery_id',(int)$lottery['id'])->where('status',1)->whereNull('deleted_at')->column('id'));
        foreach(Db::name('lottery_odds_categories')->where('lottery_id',(int)$lottery['id'])->where('is_playable',1)->where('status',1)->whereNull('deleted_at')->column('id') as $id) $valid[]=1000000000+(int)$id;
        ...
        $node=OrganizationHierarchy::nodeForSession($session);if(!$node)throw new \RuntimeException('当前账号尚未绑定组织');$key=$this->settingKey((int)$lottery['id']);$settings=$this->organizationSettings($session);$settings[$key]=$result;$settings[self::FOLLOW_SETTING_KEY]=$follow;
        ...
        return $this->reply(['lottery'=>$lottery,'amounts'=>$result,'follow_share'=>$follow],'设置保存成功');
    }
