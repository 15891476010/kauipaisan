...
final class Organization
{
    ...
    private function catalog(int $siteId): array
    {
        $sitePermissions=AgentAuthorization::sitePermissions($siteId);
        ...
        return ['levels'=>array_map(static fn(string $level): array=>['value'=>$level,'label'=>OrganizationHierarchy::LABELS[$level]],OrganizationHierarchy::LEVELS),'permissions'=>array_map(static fn(string $code,string $label): array=>['code'=>$code,'label'=>$label],array_keys($permissions),array_values($permissions))];
    }
    ...
    private function responseForSite(int $siteId,?int $parentId=null): array
    {
        ...
        return ['nodes'=>$nodes,'members'=>$members,'accounts'=>$accounts,'catalog'=>$this->catalog($siteId),'site_max_share_rate'=>number_format($siteCap,4,'.','')];
    }
