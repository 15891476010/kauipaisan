...
final class AgentAuthorization
{
    ...
    public static function sitePermissions(int $siteId): array
    {
        if($siteId<1)return [];
        ...
        if(!is_array($settings)||!array_key_exists('agent_permissions',$settings))return ['*'];
        return self::normalize($settings['agent_permissions']);
    }
