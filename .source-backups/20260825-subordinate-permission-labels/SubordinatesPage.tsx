...
export function SubordinatesPage({ agentName }: { agentName: string }) {
  ...
  return <section className="subordinate-page">
    <div className="subordinate-location">
      ...
      <div className="subordinate-actions"><button className="active" type="button">账户列表</button>{canCreate && <><i /><button type="button" onClick={openCreate}><PlusOutlined />新增{isMemberMode ? "会员" : childLabel}</button></>}</div>
    </div>
    <div className="subordinate-content">
      ...
      <form className="member-filters" onSubmit={(event) => { event.preventDefault(); setPage(1); setApplied({ ...filters }); }}>
        ...
        <button className="member-search" type="submit"><SearchOutlined />搜索</button>{canCreate && <button className="member-create" type="button" onClick={openCreate}><PlusOutlined />新增{isMemberMode ? "会员" : childLabel}</button>}
      </form>
