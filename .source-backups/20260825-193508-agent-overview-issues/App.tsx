...
import { getAgentLineOptions, getAgentOrganizationProfile, getAgreement, getAnnouncement, getBranding, getLedgerIssues, getLotteries, type AgentOrganizationProfile, type Announcement, type LedgerIssue, type Lottery } from "./api/user";
  ...
  useEffect(() => {
    ...
    if (suppliedLottery) {
      ...
      void getLotteries().then((response) => {
        ...
        setLottery(list.find((item) => item.id === selectedId)?.name || list[0]?.name || "");
...
function AgentMain({ name, onLogout, announcement, siteName }: { name: string; onLogout: () => void; announcement: Announcement; siteName: string }) {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent("agent:lottery-changed", { detail: selected?.name || "" }));
