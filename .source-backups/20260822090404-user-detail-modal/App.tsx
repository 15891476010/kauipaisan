...
function Header({ name, logout, announcement, balances, lotteries, siteName, selectableLottery = false, selectedLotteryId, onSelectLottery }: { name: string; logout: () => void; announcement: Announcement; balances: Balances; lotteries: Lottery[]; siteName: string; selectableLottery?: boolean; selectedLotteryId?: number | null; onSelectLottery?: (id: number) => void }) {
  ...
  return (
    <>
      ...
      <header className="site-header">
        <div className="site-brand"><img className="fish-logo" src={loginLogo} alt="" /><strong>{siteName}</strong></div>
        ...
        <>
          ...
          <QuickResultTable
            ...
            onChange={(lines, reason) => {
              ...
              setGeneratedFormattedText(lines.map((line) => line.raw_text).join("\n"));
...
function SideBetRecords({ onMore, panelRight, onToggleSide }: { onMore: () => void; panelRight: boolean; onToggleSide: () => void }) {
  ...
  const displayRecords = useMemo(() => { const grouped: Array<BetRecord & { record_ids: number[] }> = []; const index = new Map<string, number>(); for (const record of records) { const key = `${record.source_text || ""}|${record.formatted_text || ""}|${record.placed_at}`; const canMerge = Boolean(record.source_text && (record.source_text.includes("福体") || record.source_text.includes("双打"))); const existing = canMerge ? index.get(key) : undefined; if (existing === undefined) { grouped.push({ ...record, record_ids: [record.id] }); if (canMerge) index.set(key, grouped.length - 1); continue; } const target = grouped[existing]; target.record_ids.push(record.id); target.bet_count += record.bet_count; target.amount = (Number(target.amount) + Number(record.amount)).toFixed(2); target.win_amount = (Number(target.win_amount) + Number(record.win_amount)).toFixed(2); target.lottery = `${target.lottery || ""}${record.lottery || ""}`; target.status = target.status === "refunded" && record.status === "refunded" ? "refunded" : target.status; target.can_refund = Boolean(target.can_refund && record.can_refund); } return grouped; }, [records]);
  const showRecord = async (record: BetRecord & { record_ids?: number[] }, mode: "detail" | "numbers") => { setDetailRecord(record); setDetailMode(mode); setDetailLoading(true); try { const ids = record.record_ids || [record.id]; const responses = await Promise.all(ids.map((id) => getBetDetails({ bet_record_id: id, page: 1, page_size: 100 }))); setDetails(responses.flatMap((response) => response.data?.data?.list || [])); } catch (error) { setDetails([]); message.error(apiErrorMessage(error,"注单详情加载失败")); } finally { setDetailLoading(false); } };
  const refund = (record: BetRecord & { record_ids?: number[] }) => modal.confirm({ title:"确认退单", content:`确定退回第 ${record.issue_no} 期注单，金额 ¥ ${record.amount} 吗？`, okText:"确认退单", cancelText:"取消", okButtonProps:{danger:true}, onOk:async()=>{ try { for (const id of record.record_ids || [record.id]) await refundBetRecord(id); message.success("退单成功"); setDetailRecord(undefined); await load(); window.dispatchEvent(new Event("profile-updated")); } catch(error) { message.error(apiErrorMessage(error,"退单失败")); } } });
  const detailGroups = useMemo(() => { const groups = new Map<string, BetDetail[]>(); for (const detail of details) { const key = detail.lottery || "其他"; const list = groups.get(key) || []; list.push(detail); groups.set(key, list); } return Array.from(groups.entries()).map(([lottery, rows]) => ({ lottery, rows })); }, [details]);
  return <><div className="side-total"><span>总金额: <b>{amountTotal}</b></span><div className="side-actions"><button type="button" onClick={onMore}>更多</button><button className="side-right" type="button" onClick={onToggleSide}><img className={panelRight ? "is-left" : ""} src={arrowRightIcon} alt="" aria-hidden="true" />{panelRight ? "居左" : "居右"}</button></div></div><div className="side-record-list">{displayRecords.map((record)=><article className={`side-record-item${record.status==="refunded"?" refunded":""}`} key={record.record_ids.join("-")}><time>{record.placed_at}</time><div className="side-record-text">{record.status === "refunded" && <span className="side-refunded-label">已退码</span>}<p>{record.source_text || "-"}</p></div><footer><strong>{record.amount}</strong>{record.status !== "refunded" && <div><button type="button" className="side-record-action detail" onClick={()=>showRecord(record,"detail")}>详</button><button type="button" className="side-record-action numbers" onClick={()=>showRecord(record,"numbers")}>号</button><button type="button" className="side-record-action refund" disabled={!record.can_refund || loading} title={record.can_refund?`开奖时间 ${record.open_time}`:"仅开奖前可以退单"} onClick={()=>refund(record)}>退</button></div>}</footer></article>)}</div><Modal className="record-detail-modal" open={Boolean(detailRecord)} title={detailRecord?`${detailMode==="detail"?"下注详情":"投注号码"}`:""} footer={null} onCancel={()=>setDetailRecord(undefined)} width={1000}>{detailLoading?<div className="record-detail-loading">加载中...</div>:details.length?(detailMode==="detail"?<div className="record-detail-reference"><div className="record-detail-tabs">{detailGroups.map((group)=><span key={group.lottery}>{group.lottery}</span>)}</div>{detailGroups.map((group)=><section className="record-detail-group" key={group.lottery}><h3>{group.lottery}</h3><h4>{((group.rows[0]?.category && !["体","福","福彩3D","排列三"].includes(group.rows[0].category)) ? group.rows[0].category : ((group.rows[0]?.source_text||"").match(/(对子|双飞|胆拖|直选|组六(?:多码|五码)|组三(?:多码|五码)|和值[^ ]*)/)?.[1] || group.rows[0]?.play_type || "投注")).replace("组六五码","组六多码")}</h4><table><thead><tr><th>号码</th><th>金额</th><th>赔率</th><th>中奖</th></tr></thead><tbody>{group.rows.map((detail)=><tr key={detail.id}><td>{detail.number_text||"-"}</td><td>{detail.amount}</td><td>{detail.odds||"-"}</td><td>{detail.win_amount||"---"}</td></tr>)}</tbody></table></section>)}</div>:<div className="record-detail-list">{details.map((detail)=><div className="record-detail-line" key={detail.id}><span className={detail.lottery==="福彩3D"?"fu":"ti"}>{detail.lottery==="福彩3D"?"福":detail.lottery==="排列三"?"体":detail.lottery||"-"}</span><div className="record-number-chips">{detail.number_text.split(/[\s,，]+/).filter(Boolean).map((number,index)=><i key={`${number}-${index}`}>{number}</i>)}</div></div>)}</div>):<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无数据" />}</Modal></>;
}
function StopDrop() {
  const { message } = AntdApp.useApp();
...
function Main({ name, logout, siteName, forcePasswordChange = false, onPasswordChanged }: { name: string; logout: () => void; siteName: string; forcePasswordChange?: boolean; onPasswordChanged?: () => void }) {
  ...
  return (
    <div className="app">
      <Header name={name} logout={logout} announcement={announcement} balances={balances} lotteries={lotteries} siteName={siteName} selectableLottery={lotterySwitchPages.has(location.pathname)} selectedLotteryId={selectedLotteryId} onSelectLottery={setSelectedLotteryId} />
...
export default function App() {
  const [siteName, setSiteName] = useState("站点系统");
    ...
    <HashRouter>
      {name ? (
        agreementVisible ? (
          ...
          />
        ) : <Main name={name} logout={logout} siteName={siteName} forcePasswordChange={mustChangePassword} onPasswordChanged={() => setMustChangePassword(false)} />
      ) : (
        <Routes>
          <Route
            path="*"
