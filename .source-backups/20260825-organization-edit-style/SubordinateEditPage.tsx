...
export function SubordinateEditPage({ agentName }: { agentName: string }) {
  ...
  useEffect(() => {
    ...
    void request.then((response) => {
      if (organizationMode) {
        ...
        setForm((current) => ({ ...current, display_name: node.display_name || node.name, password: "", credit_balance: node.credit_limit, interception_rate: node.share_rate || "0" }));
        return;
      }
  ...
  async function submit() {
    if (organizationMode) {
      ...
      setSaving(true);
      try {
        await updateAgentOrganization(memberId, { display_name: form.display_name.trim(), name: form.display_name.trim(), password: form.password, credit_limit: Number(form.credit_balance), share_rate: Number(form.interception_rate), max_share_rate: Number(form.interception_rate), status: form.account_state === "enabled" ? 1 : 0 });
  ...
  if (organizationMode) {
    return <section className="subordinate-page subordinate-edit-page"><div className="subordinate-location"><div className="subordinate-path"><strong>位置</strong><DoubleRightOutlined /><span>下级管理</span><DoubleRightOutlined /><span>修改下级</span></div><div className="subordinate-actions"><button type="button" onClick={() => navigate("/subordinates")}>账户列表</button><i /><button className="active" type="button">修改下级</button></div></div>{loading ? <div className="subordinate-edit-state">正在加载下级资料...</div> : loadError || !organization ? <div className="subordinate-edit-state error"><span>{loadError || "下级不存在"}</span><button type="button" onClick={() => navigate("/subordinates")}>返 回</button></div> : <div className="subordinate-edit-shell"><div className="edit-credit-summary"><strong>{agentName}</strong><span>下级层级：</span><b>{organization.level_label}</b><span>当前额度：</span><b>{organization.credit_limit}</b><span>剩余分数：</span><b>{organization.balance || "0.00"}</b></div><section className="edit-account-panel"><div className="edit-field readonly"><label>账号名</label><strong>{organization.username || "-"}</strong></div><div className="edit-field"><label htmlFor="edit-org-name">名称：</label><input id="edit-org-name" maxLength={120} value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} /></div><div className="edit-field"><label htmlFor="edit-org-password">密码：</label><input id="edit-org-password" type="password" autoComplete="new-password" placeholder="留空不修改" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /></div><div className="edit-field"><label htmlFor="edit-org-credit">分数额度：</label><input id="edit-org-credit" type="number" min="0" step="0.01" value={form.credit_balance} onChange={(event) => setForm({ ...form, credit_balance: event.target.value })} /></div><div className="edit-field"><label htmlFor="edit-org-share">实际占成：</label><input id="edit-org-share" type="number" min="0" step="0.0001" value={form.interception_rate} onChange={(event) => setForm({ ...form, interception_rate: event.target.value })} /></div><div className="edit-state-group"><b>账号状态：</b>{[["enabled", "启用"], ["disabled", "停用"]].map(([value, label]) => <label key={value}><input type="radio" name="organization-state" value={value} checked={form.account_state === value} onChange={() => setForm({ ...form, account_state: value as EditState["account_state"] })} />{label}</label>)}</div></section><div className="edit-average-rebate"><button className="inline-submit submit" type="button" disabled={saving} onClick={() => void submit()}>{saving ? "提交中" : "提 交"}</button><button className="inline-submit" type="button" onClick={() => navigate("/subordinates")}>返 回</button></div></div>}</section>;
  }
