...
final class QuickEntryParser
{
    ...
    /**
     * Stage-1 scanner used by the rewritten pipeline. It never rewrites the
     * user's source and never emits settlement syntax; callers can use the
     * context for diagnostics before dispatching to a play handler.
     */
    public function scanStages(string $text, string $lottery, float $unitStake = 2.0): QuickEntryStageContext
    {
        ...
        [$category, $numberSource] = QuickEntryStages::lottery($normalized, $lottery);
        $numbers = QuickEntryStages::numbers($numberSource);
        $plays = QuickEntryStages::plays($normalized);
        $lotteryCode = QuickEntryStages::lotteryCode($category);
        $playCodes = QuickEntryStages::playCodes($plays);
        [$amount, $amountUnit] = QuickEntryStages::amount($normalized, $unitStake);
        if ($plays !== [] && $numbers === []) return new QuickEntryStageContext($source, $lottery, $category, [], $plays, $amount, $amountUnit, ParseStage::NUMBER->value, '未识别到有效号码', $lotteryCode, $playCodes, ParseStage::NUMBER);
        if ($plays !== [] && $amount === null && !preg_match('/全包|和值|跨度|定位|飞|转/u', $normalized)) return new QuickEntryStageContext($source, $lottery, $category, $numbers, $plays, null, null, ParseStage::AMOUNT->value, '未识别到有效金额', $lotteryCode, $playCodes, ParseStage::AMOUNT);
        return new QuickEntryStageContext($source, $lottery, $category, $numbers, $plays, $amount, $amountUnit, null, null, $lotteryCode, $playCodes);
    }
    ...
    public function parse(string $text, string $lottery, float $unitStake = 2.0): array
    {
        ...
        $text = preg_replace('/(?:🈴|合计|合)?\s*\d+(?:\.\d+)?\s*[+*×]\s*\d+(?:\.\d+)?\s*=\s*\d+(?:\.\d+)?\s*(?:元|米|块)?/u', '', $text) ?? $text;
        ...
        for ($index = 0; $index < $lineCount; $index++) {
            if (isset($batchSegments[$index])) {
                foreach ($batchSegments[$index]['rows'] as $row) {
                    $row['input_text'] = (string)($row['raw_text'] ?? '');
                    $row['id'] = $lineId++;
            ...
            if (isset($parsed[0]) && is_array($parsed[0])) {
                foreach ($parsed as $row) {
                    $row['input_text'] = $rawOriginal;
                    $row['id'] = $lineId++;
                    if (($row['status']??'')==='success') $row['ast']=$this->astFor($row,$lottery);
                    $result[] = $row;
                }
            } else {
                $parsed['input_text'] = $rawOriginal;
                $parsed['id'] = $lineId++;
    ...
    private function parseLine(string $rawOriginal, string $lottery, int $lineId, float $unitStake): array
    {
        ...
        $staged = $this->parseSimpleStaged($rawOriginal, $raw, $stages, $lineId, $unitStake);
        if ($staged !== null) return $staged;
        if (preg_match('/^\s*(?:福体|福|体)?\s*两\s*单\s*一\s*组\s*((?:\d{3}\s+)+)(\d{1,3})\s*$/u', $rawOriginal, $bare)) {
            preg_match_all('/\d{3}/', $bare[1], $nm); $nums=array_values(array_unique($nm[0]??[])); $total=(float)$bare[2]; $cat=$category==='福体'?2:1; $rows=[];
            foreach([['直','直',2],['组六','组',1]] as [$play,$suffix,$weight]) $rows[]=['id'=>$lineId,'raw_text'=>$rawOriginal,'status'=>'success','reason'=>null,'number_text'=>implode(' ',array_map(static fn(string $n):string=>$n.$suffix,$nums)),'category'=>$category,'amount'=>number_format($total*$weight/3*$cat,2,'.',''),'count'=>count($nums)*$cat,'play_type'=>$play,'settlement_text'=>implode(' ',$nums).' '.$play.' '.$category];
            return $rows;
        }
        if (preg_match('/^\s*(福体|福|体)?\s*(\d{1,10})\s*(组三|组六|组3|组6)\s*(?:一|1)倍\s*$/u', $rawOriginal, $g3)
            ...
            $m = $g3 ?: $g3m; $sel = $m[2]; $isThree = in_array($m[3], ['组三','组3'], true); $cat = $category === '福体' ? 2 : 1; $base = strlen($sel) === 3 ? 2.0 : 10.0; $name = strlen($sel) === 3 ? ($isThree ? '组三' : '组六') : ($isThree ? '组三'.$this->countToken((string)strlen($sel)).'码' : '组六'.$this->countToken((string)strlen($sel)).'码');
        ...
        if (preg_match('/^\s*(福体|福|体)?\s*(\d{1,10})\s*(组三|组六|组3|组6)\s*([\d.一二两三四五六七八九十]+)倍\s*$/u', $rawOriginal, $gm)) {
            $sel=$gm[2]; $isThree=in_array($gm[3],['组三','组3'],true); $cat=$category==='福体'?2:1; $mult=ctype_digit($gm[4])?(float)$gm[4]:(float)$this->countToken($gm[4]); $base=strlen($sel)===3?2.0:10.0; $name=strlen($sel)===3?($isThree?'组三':'组六'):($isThree?'组三'.$this->countToken((string)strlen($sel)).'码':'组六'.$this->countToken((string)strlen($sel)).'码');
        ...
        if (preg_match('/^\s*(福体|福|体)?\s*(\d{3})\s*(?:组六|组6)\s+(?:一|1)倍\s*$/u', $rawOriginal, $threeGroupOne)) {
            ...
            return [[ 'id'=>$lineId,'raw_text'=>$rawOriginal,'status'=>'success','reason'=>null,'number_text'=>$threeGroupOne[2].'组','category'=>$category,'amount'=>number_format(2*$cat,2,'.',''),'count'=>$cat,'play_type'=>'组六','settlement_text'=>$threeGroupOne[2].' 组六 '.$category,'amount_type'=>'multiplier','multiplier'=>1.0 ]];
        }
        ...
        if ($groupedDigitSet !== null) {
            if (preg_match('/组六/u', $raw) && preg_match('/(\d+(?:\.\d+)?)\s*倍/u', $raw, $mul) && preg_match('/\d{4,}/u', $raw)) {
                foreach ($groupedDigitSet as &$row) $row['amount'] = number_format((float)$mul[1] * 10.0 * ($category === '福体' ? 2 : 1), 2, '.', '');
                unset($row);
            }
    ...
    private function parseMixedGroupMultiplierBet(string $rawOriginal, string $raw, string $category, int $lineId, float $unitStake): ?array
    {
        ...
        foreach ($specs as $family => $multiplier) {
            foreach ($selections as $selection) {
                ...
                $playType = $identity['name'];
                $amount = $multiplier * 10.0 * $categoryCount;
                $rows[] = [
                    'id' => $lineId,
                    'raw_text' => $rawOriginal,
    ...
    private function parseSimpleStaged(string $rawOriginal,string $raw,array $stages,int $lineId,float $unitStake): ?array
    {
        if (preg_match('/^\s*(?:福体|福|体)?\s*((?:\d{3}\s+)+\d{3})\s*(直|单|组|组三|组六)\s*(?:各|每)?\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛|倍|注)\s*$/u', $rawOriginal, $m) !== 1
            && preg_match('/^\s*(?:福体|福|体)?\s*((?:\d{3}\s+)+\d{3})\s*(直|单|组|组三|组六)\s*(?:各|每)?\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛|倍|注)\s*$/u', $raw, $m) !== 1) return null;
        $numbers = preg_split('/\s+/', trim($m[1])) ?: [];
        $play = $m[2] === '单' ? '直' : $m[2];
        $category = (string)($stages['category'] ?? '福'); $cat = $category === '福体' ? 2 : 1;
        $value = (float)$m[3]; $unit = (string)($m[4] ?? '');
        $per = $unit === '倍' ? (($play === '直' || count($numbers) === 1) ? $value * $unitStake : $value * 10.0) : $this->rules->amountWithUnit($value, $unit, $unitStake);
        $suffix = in_array($play, ['组','组三','组六'], true) ? '组' : '直';
        $actual = $play === '组' ? '组' : $play;
        return [[ 'id'=>$lineId,'raw_text'=>$rawOriginal,'status'=>'success','reason'=>null,'number_text'=>implode(' ',array_map(static fn(string $n):string=>$n.$suffix,$numbers)),'display_number_text'=>implode(' ',$numbers),'category'=>$category,'amount'=>number_format($per*count($numbers)*$cat,2,'.',''),'count'=>count($numbers)*$cat,'stake_count'=>count($numbers)*$cat,'code_count'=>count($numbers)*$cat,'play_type'=>$actual,'settlement_text'=>implode(' ',$numbers).' '.$actual.'各'.number_format($per,2,'.','').'元 '.$category ]];
    }
    ...
    private function parseDragBets(string $rawOriginal,string $raw,string $category,int $lineId,float $unitStake): ?array
    {
        ...
        if ($specs === [] && str_contains($raw, '全胆拖') && preg_match($this->rules->pattern('per_unit_amount'), $raw, $amount)) {
            $specs[] = ['play'=>'单选', 'amount'=>$this->rules->amountWithUnit((float)$amount[1], (string)($amount[2] ?? ''), $unitStake)];
        }
    ...
    private function parseCatalogBet(string $rawOriginal, string $raw, string $category, int $lineId, float $unitStake): ?array
    {
        ...
        if (preg_match('/^\s*(?:福体|福|体)?\s*([0-9]{2,10})\s*([一二两三四五六七八九十\d]+)码\s*(组三|组六)\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛|倍)?\s*$/u', $raw, $compactPrefix)) {
            ...
            $amount=((string)($compactGroup[4]??'')==='倍') ? (float)$compactGroup[3]*10.0 : $this->rules->amountWithUnit((float)$compactGroup[3],(string)($compactGroup[4]??''),$unitStake);
        ...
        if (strlen($selection) > 3 && (str_contains($playType, '组六') || str_contains($playType, '组三')) && preg_match('/(\d+(?:\.\d+)?)\s*倍/u', $raw, $mul)) {
            $amount = (float)$mul[1] * 10.0;
        }
        $categoryCount = $category === '福体' ? 2 : 1;
