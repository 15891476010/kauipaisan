...
export type ThirdPartyQuickEntryAccount = { id: string; username: string; password: string; rate_window_seconds?: number | null; rate_limit_calls?: number | null; freeze_seconds?: number | null };
...
export const testThirdPartyQuickEntryConfig = (payload?: { site_id?: number; text?: string; lottery?: string }) =>
  http.post<never, Envelope<{ code: number; total_amount: number | string | null; total_count: number | string | null; result: Record<string, unknown> }>>('/admin/system-settings/third-party-quick-entry/test', payload);
