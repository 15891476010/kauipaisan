...
export function QuickResultTable({ lines, sourceText: _sourceText, onChange, onConfirmMismatch }: QuickResultTableProps) {
  ...
  // V2 may return several semantic rows for one multi-line ticket while
  // retaining the complete source text on every row. Those rows are already
  // the parser result; mapping them back to physical lines duplicates rows
  // (and assigns misleading repeated indexes) when the source contains a
  // blank line.
  const parserReturnedWholeSource = lines.length > 0 && lines.every((line) =>
    String(line.input_text || line.raw_text || "").includes("\n"),
  );
  ...
  // a misleading “新” row.
  if ((hasPhysicalBlank || physicalExpanded) && !parserReturnedWholeSource) {
    ...
    sourcePhysical.forEach((raw, physicalIndex) => {
      ...
      // Header-only lines such as “福彩第2026236期” are context, not a
      // bet row. They do not have a standalone three-digit selection or a
      // play marker, so leave them out without consuming the next semantic
      // parser row; otherwise every following row is shifted and duplicated.
      if (!/(?<!\d)\d{3}(?!\d)/u.test(raw) && !/(直|单|组|复式|定位|和值|跨度|胆拖|飞)/u.test(raw)) {
        return;
      }
      // A single physical sentence can produce several semantic rows (for
      // example the 直 and 组 legs). Group the contiguous parser rows that
      // came from that sentence, then render them with the same source key so
      // displayGroups can combine them into one visible table row. The old
      // implementation compared normalized text literally and consumed one
      // semantic row per physical line, which duplicated rows and produced
      // 14 rows for an 8-line paste.
      if (semanticIndex >= lines.length) return;
      const firstSource = String(lines[semanticIndex].input_text || lines[semanticIndex].raw_text || "").trim();
      const sourceKey = firstSource || raw.trim();
      const members: QuickEntryLine[] = [];
      while (semanticIndex < lines.length) {
        const candidate = lines[semanticIndex];
        const candidateSource = String(candidate.input_text || candidate.raw_text || "").trim();
        if (members.length > 0 && candidateSource !== sourceKey) break;
        members.push(candidate);
        semanticIndex += 1;
      }
      members.forEach((candidate) => expandedLines.push({
        ...candidate,
        id: physicalIndex + 1,
        raw_text: raw,
        input_text: raw,
        __sourceIds: [candidate.id],
        __detailLines: [candidate],
      }));
    });
    while (semanticIndex < lines.length) {
      const candidate = lines[semanticIndex++];
  ...
  const detailSectionsRaw = detailLines.flatMap((line) => {
    ...
    const occurrenceWeight = occurrences.length > 0 && stakeCount > 0 ? stakeCount / occurrences.length : 1;
  ...
  const detailCount = displayCount(detailLines);
                        ...
                        <Fragment key={`${section}-row-${rowIndex}`}>
                          {Array.from({ length: 4 }, (_, index) => row[index] || null).map((number, index) => (
                            <div className={`row-label-container${number ? " has-amount" : ""}`} key={`${section.category}-${section.title}-${rowIndex}-${index}`}>
                              ...
                              <span className="label-wrapper">{number ? formatAmount((section.amounts[number] || 0).toFixed(2)) : "--"}</span>
                            </div>
                          ))}
                        </Fragment>
                      ))}
