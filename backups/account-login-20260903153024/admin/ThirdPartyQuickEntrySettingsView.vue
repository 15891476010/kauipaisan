...
function statusText(status?: string) { return ({ calling: '调用中', success: '成功', failed: '失败', login_failed: '登录失败' } as Record<string, string>)[status || ''] || '未调用' }
onMounted(load)
</script>
    ...
    <el-card shadow="never"><el-form label-position="top">
      ...
      <div v-for="(account, index) in form.accounts" :key="account.id" class="account-card">
        ...
        <div class="account-runtime">
          ...
          <span class="ak">AK：{{ account.ak || '尚未登录' }}</span><span v-if="account.last_error" class="runtime-error">{{ account.last_error }}</span>
        </div>
      </div>
