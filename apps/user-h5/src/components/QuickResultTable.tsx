...
function QuickResultTableInner({ lines, sourceText: _sourceText, onChange, onConfirmMismatch }: QuickResultTableProps) {
  ...
  const numberTokens = (line: QuickEntryLine) => {
    ...
    // 定位行的号码按“位置+数字”显示（8XX→百8、16X→百1 十6、
    // XX4→个4），不直接透出 X/口 内部占位符。
  ...
  const playLabel = (line: QuickEntryLine) => {
    ...
    const locatorCount = ["百", "十", "个"].filter((position) => new RegExp(`${position}位?\\s*[0-9０-９]`, "u").test(allText)).length;
    ...
    if (/^(?:百十个|口口口)$/u.test(play) || /三码定位/u.test(allText)) return "三码定位";
                        ...
                        <Fragment key={`${section}-row-${rowIndex}`}>
                          {Array.from({ length: 4 }, (_, index) => row[index] || null).map((number, index) => (
                            <div className={`row-label-container${number && section.frequency[number] > 1 ? " has-amount" : ""}`} key={`${section.category}-${section.title}-${rowIndex}-${index}`}>
                              ...
                              <span className="label-wrapper">{number ? formatAmount(section.amounts[number] ?? section.unitAmount * (section.frequency[number] || 1)) : "--"}</span>
                            </div>
                          ))}
                        </Fragment>
                      ))}
