...
export function Login({ onLogin, siteName }: { onLogin: (name: string) => void; siteName: string }) {
  ...
  async function authenticate() {
    ...
    try {
      ...
      localStorage.setItem("user_must_change_password", response.data.data?.user?.must_change_password ? "1" : "0");
      onLogin(name);
    } catch (reason) {
  ...
  async function submitCaptcha() {
    try {
      ...
      message.error(apiErrorMessage(reason, "验证码错误或已失效"));
      setCaptcha("");
      await requestCaptcha();
    }
  }
  ...
  return (
    <div className="login">
      ...
      <Modal className="captcha-login-modal" title="请输入图片验证码" open={showCaptcha} onCancel={() => setShowCaptcha(false)} destroyOnHidden footer={<div className="captcha-footer"><Button htmlType="button" type="primary" loading={busy} onClick={() => void submitCaptcha()}>登 录</Button><Button htmlType="button" onClick={() => setShowCaptcha(false)}>关 闭</Button></div>} width={396} style={{ top: "calc(50dvh - 167px)" }}>
        <div className="captcha-box">
          <div className="captcha-equation"><img className="captcha-image" src={captchaImage} alt="算术验证码" onClick={() => void requestCaptcha()} /><b>=</b><strong>{captcha}</strong><Button htmlType="button" type="link" onClick={() => void requestCaptcha()}>换题</Button></div>
