...
import { App as AntdApp, Button, DatePicker, Empty, Input, Modal, Spin } from "antd";
...
import { acceptAgentAgreement, clearAgentAgreement, hasAcceptedAgentAgreement } from "./utils/agreementSession";
  ...
  return (
    <section className="overview-page">
      ...
      <form className={`overview-filters overview-filters-${activeTab === "总货概览" ? "overview" : activeTab === "查看退码" ? "refund" : activeTab === "投注明细" ? "bets" : activeTab === "中奖明细" ? "winning" : "details"}`} onSubmit={(event) => { event.preventDefault(); setPage(1); setQueryVersion((value) => value + 1); }}>
        {activeTab === "投注明细" ? <>
          <fieldset className="status-filter"><legend>中奖</legend><select className="overview-select" value={winningStatus} onChange={event=>setWinningStatus(event.target.value)}><option value="all">全部</option><option value="won">中奖</option><option value="unwon">未中</option></select></fieldset>
          ...
          {activeTab !== "总货概览" && activeTab !== "查看退码" ? <fieldset className="range-filter"><legend>列出</legend><div><select className="overview-select metric-select" value={metric} onChange={event=>setMetric(event.target.value as typeof metric)}><option value="odds">赔率</option><option value="amount">金额</option></select><Input className="overview-range-input" size="small" value={min} onChange={(event) => setMin(event.target.value.replace(/[^\d.]/g, ""))} /><span>至</span><Input className="overview-range-input" size="small" value={max} onChange={(event) => setMax(event.target.value.replace(/[^\d.]/g, ""))} /></div></fieldset> : null}
          {activeTab !== "总货概览" && activeTab !== "查看退码" ? <fieldset className="category-filter"><legend>分类</legend><select className="overview-select" value={category} onChange={event=>setCategory(event.target.value)}>{["所有", "直选", "组三", "组六", "定位", "和值", "跨度"].map(value=><option key={value} value={value}>{value}</option>)}</select></fieldset> : null}
          {activeTab === "中奖明细" ? <><fieldset className="source-filter"><legend>来源</legend><select className="overview-select" value={source} onChange={event=>setSource(event.target.value)}><option value="全部">全部</option><option value="quick">快录</option></select></fieldset><fieldset className="source-filter"><legend>设备</legend><select className="overview-select" defaultValue="全部"><option value="全部">全部</option><option value="快录网">快录网</option></select></fieldset></> : activeTab === "总货概览" ? <fieldset className="source-filter"><legend>来源：</legend><select className="overview-select" value={source} onChange={event=>setSource(event.target.value)}><option value="全部">全部</option><option value="quick">快录</option></select></fieldset> : null}
        </>}
      ...
      <section className="overview-table-panel">
        <div className="overview-table-title">
          ...
          <select className="overview-issue-select" value={startDate} onChange={(event) => setStartDate(event.target.value)} aria-label="开始日期" disabled={issuesLoading || issues.length === 0}>{issues.length === 0 ? <option value="">{issuesLoading ? "正在加载期号" : "暂无可用期号"}</option> : issues.map((item) => <option key={item.issue_no} value={item.issue_no}>{formatIssueOption(item)}</option>)}</select>
          <span>至</span>
          <select className="overview-issue-select" value={endDate} onChange={(event) => setEndDate(event.target.value)} aria-label="结束日期" disabled={issuesLoading || issues.length === 0}>{issues.length === 0 ? <option value="">{issuesLoading ? "正在加载期号" : "暂无可用期号"}</option> : issues.map((item) => <option key={item.issue_no} value={item.issue_no}>{formatIssueOption(item)}</option>)}</select>
        </div>
        ...
        <div className="overview-pagination"><span>总计：<b>{total}</b> 条数据</span><button type="button" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>‹</button><strong>{page}</strong><button type="button" disabled={page >= Math.max(1, Math.ceil(total / pageSize))} onClick={() => setPage((value) => value + 1)}>›</button><select className="overview-page-size" value={pageSize} onChange={event=>{setPageSize(Number(event.target.value));setPage(1)}}><option value="10">10 条/页</option><option value="40">40 条/页</option><option value="100">100 条/页</option></select></div>
      </section>
...
export default function App() {
  ...
  const [agreementVisible, setAgreementVisible] = useState(() => {
    const token = localStorage.getItem("agent_token");
    return Boolean(token && localStorage.getItem("agent_name") && !hasAcceptedAgentAgreement(token));
  });
  ...
  const clearSession = () => { clearAgentAuthQuery(); localStorage.removeItem("agent_token"); localStorage.removeItem("agent_name"); localStorage.removeItem("agent_permissions"); localStorage.removeItem("agent_is_subaccount"); localStorage.removeItem("agent_organization_level"); localStorage.removeItem("agent_level_label"); localStorage.removeItem("agent_must_change_password"); clearAgentAgreement(); setAgreementVisible(false); setMustChangePassword(false); setName(""); };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (lineSwitch) acceptAgentAgreement();
    else if (!hasAcceptedAgentAgreement(token)) clearAgentAgreement();
    clearAgentAuthQuery();
    setName(agentName);
    setAgreementVisible(!hasAcceptedAgentAgreement(token));
  }, []);
  useEffect(() => {
