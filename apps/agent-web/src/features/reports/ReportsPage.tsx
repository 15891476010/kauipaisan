...
import { getAgentMonthlyReport, getAgentReport, getAgentReportIssues, type AgentMonthlyReportRow, type AgentReportIssue, type AgentReportMetrics } from '../../api/user';
...
export function ReportsPage({lottery}:{lottery:string}) {
  ...
  const [lotteries,setLotteries]=useState(['福彩3D','排列三']); const [summary,setSummary]=useState(emptyMetrics); const [rows,setRows]=useState<AgentMonthlyReportRow[]>([]); const [loading,setLoading]=useState(true);
  ...
  useEffect(()=>{let active=true;setLoading(true);const task=mode==='summary'?getAgentReport(params):getAgentMonthlyReport(params);void task.then(response=>{if(!active||!response.data.data)return;if(mode==='summary'){setSummary((response.data.data as {summary:AgentReportMetrics}).summary);setRows([]);}else{const data=response.data.data as {list:AgentMonthlyReportRow[];total:AgentReportMetrics};setRows(data.list);setSummary(data.total);}}).catch((reason:unknown)=>{if(active){setSummary(emptyMetrics);setRows([]);void message.error(apiErrorMessage(reason,'报表加载失败'));}}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[mode,params,message]);
  ...
  return <section className="agent-reports-page">
    ...
    {mode==='summary'&&<div className="agent-reports-filters">{lotteryOptions.map(item=><label key={item.value}><input type="checkbox" checked={lotteries.includes(item.value)} onChange={event=>toggleLottery(item.value,event.target.checked)}/>{item.label}</label>)}<input type="date" aria-label="开始日期" value={from} onChange={event=>setFrom(event.target.value)}/><span>至</span><input type="date" aria-label="结束日期" value={to} onChange={event=>setTo(event.target.value)}/></div>}
    ...
    {mode==='monthly'&&<div className="agent-reports-month-range"><select aria-label="月报开始期号" value={fromIssue} onChange={event=>{const issue=issues.find(item=>item.issue_no===event.target.value);setFromIssue(event.target.value);if(issue)setFrom(issue.date)}}>{issues.map(item=><option key={`from-${item.issue_no}`} value={item.issue_no}>{`${Number(item.date.slice(5,7))}-${Number(item.date.slice(8,10))}(${item.issue_no})`}</option>)}</select><span>至</span><select aria-label="月报结束期号" value={toIssue} onChange={event=>{const issue=issues.find(item=>item.issue_no===event.target.value);setToIssue(event.target.value);if(issue)setTo(issue.date)}}>{issues.map(item=><option key={`to-${item.issue_no}`} value={item.issue_no}>{`${Number(item.date.slice(5,7))}-${Number(item.date.slice(8,10))}(${item.issue_no})`}</option>)}</select></div>}
    <div className="agent-reports-card">{loading?<div className="agent-reports-loading"><Spin/></div>:<ReportTable mode={mode} rows={rows} summary={summary}/>}</div>
  </section>;
}

function ReportTable({mode,rows,summary}:{mode:ReportMode;rows:AgentMonthlyReportRow[];summary:AgentReportMetrics}) {
  return <div className="agent-reports-table-scroll"><table className="agent-reports-table"><thead><tr><th rowSpan={2}>{mode==='summary'?'会员':'期号'}</th><th colSpan={5} className="member-group">会员</th><th colSpan={5} className="agent-group">代理</th><th colSpan={2} className="platform-group">总代理</th></tr><tr><th>笔数</th><th>总投</th><th>总中</th><th>总赚水</th><th>盈亏</th><th>占成金额</th><th>占成盈亏</th><th>离线赚水</th><th>总赚水</th><th>总盈亏</th><th>总投</th><th>盈亏</th></tr></thead><tbody>{mode==='monthly'&&rows.map(row=><MetricRow key={row.issue_no} label={row.issue_no} metrics={row.summary}/>)}<MetricRow label="合计" metrics={summary} total/></tbody></table></div>;
}
