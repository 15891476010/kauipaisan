...
const emptySummary: InterceptionSummary = { bet_amount: '0', intercepted_amount: '0', rebate: '0', water: '0', win_amount: '0', profit: '0' };
...
function DetailsTable({ rows, summary }: { rows: InterceptionRow[]; summary: InterceptionSummary }) {
  return <div className="interception-table-scroll"><table className="interception-table"><thead><tr><th>注单编号</th><th>会员</th><th>下单时间</th><th>号码</th><th>下注金额</th><th>占成</th><th>拦货金额</th><th>赔率</th><th>回水</th><th>明水</th><th>中奖</th><th>盈亏</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{row.order_no}</td><td>{row.member}</td><td>{row.placed_at}</td><td><b>{row.number}</b><small>{row.category}</small></td><td>{row.bet_amount}</td><td>{row.share_rate}</td><td>{row.intercepted_amount}</td><td>{row.odds}</td><td>{row.rebate}</td><td>{row.water || '0'}</td><td>{row.win_amount}</td><td className={Number(row.profit) < 0 ? 'negative' : 'positive'}>{row.profit}</td></tr>)}<tr className="interception-total"><td colSpan={4}>合计</td><td>{summary.bet_amount}</td><td></td><td>{summary.intercepted_amount}</td><td></td><td>{summary.rebate}</td><td>{summary.water}</td><td>{summary.win_amount}</td><td>{summary.profit}</td></tr></tbody></table></div>;
}
...
function PlateView({ groups, activeGroup, setActiveGroup, issues, issue, setIssue, loading, refresh }: { groups: InterceptionPlateGroup[]; activeGroup: string; setActiveGroup: (value: string) => void; issues: InterceptionIssue[]; issue: string; setIssue: (value: string) => void; loading: boolean; refresh: () => void }) {
  ...
  return <><div className="interception-plate-tabs">{groups.map((group) => <button type="button" key={group.category} className={activeGroup === group.category ? 'active' : ''} onClick={() => setActiveGroup(group.category)}>{group.category}</button>)}</div><div className="interception-plate-toolbar"><p>提示：当前盘面显示每个玩法的拦货额度、已拦金额与剩余额度。</p><IssueSelect label="期号" value={issue} issues={issues} onChange={setIssue}/><Button type="default" icon={<ReloadOutlined />} onClick={refresh}>刷新</Button></div><div className="interception-plate-card">{loading ? <Loading/> : selected?.items.length ? <div className="interception-plate-grid">{selected.items.map((item) => <article key={item.odds_id}><header><strong>{item.name}</strong><span>赔率 {item.odds}</span></header><dl><div><dt>设置额度</dt><dd>{item.limit}</dd></div><div><dt>已拦金额</dt><dd>{item.used}</dd></div><div><dt>剩余额度</dt><dd>{item.remaining}</dd></div></dl>{item.numbers.length > 0 && <ul>{item.numbers.map((number) => <li key={number.number}><span>{number.number}</span><b>{number.used}</b></li>)}</ul>}</article>)}</div> : <EmptyState/>}</div></>;
}
