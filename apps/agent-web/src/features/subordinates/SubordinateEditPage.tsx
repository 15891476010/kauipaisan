...
export function SubordinateEditPage({ agentName }: { agentName: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setLoadError("");
    if (!canUpdate) { setLoadError("当前未分配修改下级权限"); setLoading(false); return; }
    if (!Number.isInteger(memberId) || memberId < 1) { setLoadError("会员编号无效"); setLoading(false); return; }
    const request = organizationMode ? getAgentOrganizations(contextId ? { organization_id: contextId } : undefined) : getAgentMember(memberId);
    void request.then((response) => {
      if (organizationMode) {
        ...
        setForm((current) => ({ ...current, display_name: node.display_name || node.name, password: "", account_state: node.status === 1 ? "enabled" : "disabled", credit_balance: node.credit_limit, interception_rate: node.share_rate || "0", max_share_rate: node.max_share_rate || node.share_rate || "0" }));
        return;
      }
    ...
    return () => { active = false; };
  }, [memberId, organizationMode, contextId, canUpdate]);
  ...
  async function submit() {
    if (!canUpdate) return message.error("当前未分配修改下级权限");
    if (organizationMode) {
      ...
      try {
        ...
        message.success("下级资料已保存"); navigate(listPath);
  ...
  return <section className="subordinate-page subordinate-edit-page">
    ...
    {loading ? <div className="subordinate-edit-state">正在加载账号资料...</div> : loadError || !member ? <div className="subordinate-edit-state error"><span>{loadError || "会员不存在"}</span><button type="button" onClick={() => navigate(listPath)}>返 回</button></div> : <div className="subordinate-edit-shell">
      ...
      <div className="edit-finance-row">
        ...
        <div className="edit-average-rebate"><h5>明水（统一）：</h5><span><b>0.085</b></span><button className="inline-submit submit" type="button" disabled={saving} onClick={() => void submit()}>{saving ? "提交中" : "提 交"}</button><button className="inline-submit" type="button" onClick={() => navigate(listPath)}>返 回</button></div>
      </div>
