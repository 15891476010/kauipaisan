...
export function SubordinateFormPage() {
  ...
  const shareOptions = useMemo(() => {
    const whole = Math.floor(shareLimit);
    const values = Array.from({ length: whole + 1 }, (_, index) => index);
    if (shareLimit > whole) values.push(shareLimit);
    return values;
  }, [shareLimit]);
  ...
  async function submit() {
    ...
    const currentPermissions = organizationData?.current.permissions;
    const allowed = Array.isArray(currentPermissions)
      ? currentPermissions.includes("*") || currentPermissions.includes(requiredPermission)
      : hasAgentPermission(requiredPermission);
    if (!allowed) return message.error("当前未分配新增下级权限");
    setSaving(true);
    try {
    ...
    <section className="subordinate-page subordinate-form-page">
      ...
      {loading ? <div className="subordinate-form-loading">正在加载...</div> : loadError ? <div className="subordinate-form-loading"><div className="subordinate-load-error"><span>{loadError}</span><button type="button" onClick={() => navigate("/subordinates")}>返 回</button></div></div> : <div className="subordinate-form-shell">
        <div className="subordinate-account-form">
          ...
          {organizationData?.current.level !== "agent" && <><div className="account-name"><label htmlFor="subordinate-credit">分数额度</label><input className="ant-input subordinate-input" id="subordinate-credit" type="number" min="0" value={form.credit_limit} onChange={(event) => setForm({ ...form, credit_limit: Number(event.target.value) })} /></div><div className="account-name"><label htmlFor="subordinate-share">实际占成</label><select className="subordinate-input subordinate-select" id="subordinate-share" value={String(Math.min(form.share_rate, form.max_share_rate))} onChange={(event) => setForm({ ...form, share_rate: Number(event.target.value) })}>{shareOptions.filter((value) => value <= form.max_share_rate).map((value) => <option key={value} value={value}>{value}%</option>)}</select></div><div className="account-name"><label htmlFor="subordinate-max-share">下级最高占成</label><select className="subordinate-input subordinate-select" id="subordinate-max-share" value={String(Math.min(form.max_share_rate, shareLimit))} onChange={(event) => { const maxShare = Number(event.target.value); setForm({ ...form, max_share_rate: maxShare, share_rate: Math.min(form.share_rate, maxShare) }); }}>{shareOptions.map((value) => <option key={value} value={value}>{value}%</option>)}</select></div></>}
        </div>
