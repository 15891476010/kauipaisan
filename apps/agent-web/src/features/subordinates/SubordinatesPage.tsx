...
export function SubordinatesPage({ agentName }: { agentName: string }) {
  ...
  const canCreate = hasAgentPermission("member.create");
  const canUpdate = hasAgentPermission("member.update");
  ...
  return (
    <section className="subordinate-page">
      <div className="subordinate-location">
        ...
        <div className="subordinate-actions"><button className="active" type="button">账户列表</button>{canCreate && <><i /><button type="button" onClick={openCreate}>新增下级</button></>}</div>
      </div>
      <div className="subordinate-content">
        ...
        <form className="member-filters" onSubmit={(event) => { event.preventDefault(); setPage(1); setApplied({ ...filters }); }}>
          ...
          {canCreate && <button className="member-create" type="button" onClick={openCreate}><PlusOutlined />新增会员</button>}
        </form>
        <div className="member-table-wrap">
          <table className="member-table">
            <colgroup>
              <col className="member-col-index" />
              <col className="member-col-account" />
              <col className="member-col-parent" />
              <col className="member-col-type" />
              <col className="member-col-share" />
              <col className="member-col-credit" />
              <col className="member-col-login" />
              <col className="member-col-location" />
              <col className="member-col-status" />
              <col className="member-col-actions" />
            </colgroup>
            <thead><tr><th>编号</th><th>账号名</th><th>上级</th><th>类型</th><th>占成比例</th><th>信用额度</th><th>最近登录时间</th><th>最后登录地点</th><th>状态</th><th>内容</th></tr></thead>
            <tbody>{result.list.map((member, index) => <tr key={member.id}><td>{(page - 1) * 40 + index + 1}</td><td>{canUpdate ? <button className="member-link" type="button" onClick={() => openEdit(member)}>{member.username}(会员)</button> : <span>{member.username}(会员)</span>}</td><td><button className="member-link" type="button" onClick={() => modal.info({ title: "上级代理", content: agentName, okText: "关闭" })}>查看</button></td><td>{member.type}</td><td>代理: 0/0</td><td>{member.credit_balance}</td><td>{member.last_login_at || "-"}</td><td>{member.last_login_location || "-"}</td><td>{member.status === 1 ? "启用" : "停用"}</td><td><div className="member-actions">{canUpdate && <button className="member-link" type="button" onClick={() => openEdit(member)}>修改</button>}<button className="member-link" type="button" onClick={() => modal.info({ title: `${member.username} 月报表`, content: "暂无月报数据", okText: "关闭" })}>查看月报表</button><button className="member-link" type="button" onClick={() => modal.info({ title: "会员资料", content: <div className="member-detail"><p>用户名：{member.username}</p><p>代号：{member.display_name}</p><p>信用额度：{member.credit_balance}</p><p>可用余额：{member.available_balance}</p></div>, okText: "关闭" })}>查看</button></div></td></tr>)}</tbody>
          </table>
