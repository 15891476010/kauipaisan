...
import { App, Button, Checkbox, Empty, Form, Input, InputNumber, Modal, Space, Table, Tag } from "antd";
...
export function HierarchyPage() {
  ...
  async function saveNode() {
    try {
      ...
      else { const response = await createAgentOrganization(payload); modal.success({ title: `${levelLabel}创建成功`, content: <InitialCredentials value={response.data.data} />, okText: "我已保存", centered: true, width: 480, getContainer: getAgentModalContainer }); }
  ...
  function removeNode(row: AgentOrganizationNode) {
    Modal.confirm({ getContainer: getAgentModalContainer, title: `删除${row.level_label}`, content: `确定删除“${row.display_name || row.name}”吗？只能删除没有下级、会员和子账号的记录。`, okText: "删除", okType: "danger", cancelText: "取消", onOk: async () => { await deleteAgentOrganization(row.id); message.success(`${row.level_label}已删除`); await load(contextId || undefined); } });
  }
  ...
  return <section className="hierarchy-page">
    ...
    <Modal getContainer={getAgentModalContainer} title={editing ? `编辑${levelLabel}` : `新增${levelLabel}`} open={nodeOpen} onCancel={() => setNodeOpen(false)} onOk={() => void saveNode()} okText="保存" cancelText="取消" width={660}><div className="hierarchy-credit-notice"><span>当前剩余可分配</span><b>{availableCredit.toFixed(2)}</b>{editing && <em>当前下级最多可设置为 {maximumCredit.toFixed(2)}</em>}</div><Form form={nodeForm} layout="vertical"><div className="hierarchy-form-grid"><Form.Item name="username" label={`${levelLabel}账号`} rules={[{ required: true, message: `请输入${levelLabel}账号` }]}><Input maxLength={80} autoComplete="off" disabled={Boolean(editing)} /></Form.Item><Form.Item name="display_name" label={`${levelLabel}名称`} rules={[{ required: true, message: `请输入${levelLabel}名称` }]}><Input maxLength={120} /></Form.Item><Form.Item name="password" label={editing ? "新密码" : "登录密码"}><Input.Password placeholder={editing ? "留空不修改" : "可留空，系统自动生成"} autoComplete="new-password" /></Form.Item><Form.Item name="phone" label="手机号"><Input maxLength={30} /></Form.Item><Form.Item name="credit_limit" label="分数额度" rules={[{ required: true, message: "请输入分数额度" }]}><InputNumber min={0} max={maximumCredit} precision={2} style={{ width: "100%" }} /></Form.Item><Form.Item name="max_share_rate" label="下级最高占成"><InputNumber min={0} max={siteMaxShareRate} precision={4} addonAfter="%" style={{ width: "100%" }} /></Form.Item><Form.Item name="share_rate" label="下级实际占成"><InputNumber min={0} max={siteMaxShareRate} precision={4} addonAfter="%" style={{ width: "100%" }} /></Form.Item><Form.Item name="status" label="状态"><select><option value="1">启用</option><option value="0">停用</option></select></Form.Item></div><Form.Item name="permissions" label="功能权限"><Checkbox.Group options={permissionOptions} className="hierarchy-permissions" /></Form.Item></Form></Modal>
  </section>;
}
