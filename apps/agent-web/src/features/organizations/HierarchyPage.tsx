...
export function HierarchyPage() {
  ...
  return <section className="hierarchy-page">
    ...
    <div className="hierarchy-stats">
      <div><span>上级授予额度</span><b>{data?.current.credit.granted_credit || "0.00"}</b><small>初始额度上限</small></div>
      <div><span>当前可用余额</span><b className="available">{data?.current.credit.current_available_balance || "0.00"}</b><small>已包含结算变动</small></div>
      <div><span>直属下级额度</span><b className="allocated">{data?.current.credit.direct_child_credit || "0.00"}</b><small>仅统计直属下级</small></div>
      {data?.current.level === "shareholder" && <><div><span>未归属会员额度</span><b>{data.current.credit.unassigned_member_credit}</b><small>结算盈亏归根股东</small></div><div><span>未归属会员当前净分</span><b>{data.current.credit.unassigned_member_net_score}</b><small>较额度变动 {data.current.credit.unassigned_member_settlement_change}</small></div></>}
      <div><span>直属下级</span><b>{data?.nodes.length || 0}</b></div><div><span>在线下级</span><b>{data?.nodes.filter((item) => item.online === 1).length || 0}</b></div>
    </div>
    <div className="hierarchy-accounting-note">统计口径：上级授予额度是初始额度上限；当前可用余额会随分配、回收和结算盈亏变化，因此发生结算后不保证等于“授予额度 - 直属下级额度”。各层仅统计直属下级或直属会员，不混算整站数据。</div>
