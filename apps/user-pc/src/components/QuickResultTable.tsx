...
export function QuickResultTable({ lines, sourceText: _sourceText, onChange, onConfirmMismatch }: QuickResultTableProps) {
  ...
  const combinedSourceKey = (line: QuickEntryLine): string | null => {
    ...
    // Remove only the play marker; all digits, category markers, amount and
    // multiplier text remain part of the key and therefore must match.
    const key = source
      .replace(/组三|组六/gu, "")
      .replace(/^\s*[三六](?=\s*\d)/u, "")
      .replace(/\s+/gu, "");
  ...
  const displayTextForGroup = (group: DisplayLine[]): string => {
    ...
    // For a single-line paste retain the exact user sentence (including its
    // original spacing). This is what the third-party recognizer returned.
  ...
  const numberTokens = (line: QuickEntryLine) => {
    ...
    if (family && play !== "复式") {
      // The provider may return either the semantic play name (组三五码)
      // or the short name (组三) while keeping the selected digits in the
      // settlement/source text. Both forms represent one multi-code item.
  ...
  return (
    <>
      ...
      {displayGroups.map((group, groupIndex) => {
        ...
        // The API normally supplies suggested_amount for a total mismatch,
        // but older parser branches only returned the descriptive reason.
        // Treat both forms as the same review state so the manual-confirm
        // button is still available instead of showing a hard failure.
