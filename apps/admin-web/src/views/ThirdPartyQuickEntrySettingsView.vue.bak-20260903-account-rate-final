...
import { getThirdPartyQuickEntryConfig, saveThirdPartyQuickEntryConfig, testThirdPartyQuickEntryConfig, type ThirdPartyQuickEntryConfig } from '../api/admin'
...
async function test() {
  testing.value = true
  try { const response = await testThirdPartyQuickEntryConfig({ text: '123直1元', lottery: '福彩3D' }); ElMessage.success(`识别完成：${response.data.total_count ?? 0} 笔，${response.data.total_amount ?? 0} 元`) }
