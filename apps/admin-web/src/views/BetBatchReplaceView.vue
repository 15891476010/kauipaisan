  ...
  <div class="batch-page" v-loading="loading">
    ...
    <template v-else>
      ...
      <section class="bets-panel">
        ...
        <div v-else-if="!visibleGroups.length" class="select-tip">{{ (visibleUser && (visibleUser.display_name || visibleUser.username)) || '该用户' }} 暂无可修改的投注号码，可切换到其他用户。</div>
        <div v-else class="bet-table-wrap">
          <table class="bet-table"><thead><tr><th>用户</th><th>原始注单（可编辑）</th><th>注单金额</th></tr></thead><tbody><tr v-for="group in visibleGroups" :key="group.key"><td>{{ group.user.display_name || group.user.username }}</td><td class="source-value"><el-input type="textarea" :model-value="sourceValue(group)" :autosize="{ minRows: 2, maxRows: 8 }" @update:model-value="updateSource(group, $event)" /></td><td>¥{{ group.numbers.reduce((sum, number) => sum + Number(number.amount || 0), 0).toFixed(2) }}</td></tr></tbody></table>
        </div>
      </section>
