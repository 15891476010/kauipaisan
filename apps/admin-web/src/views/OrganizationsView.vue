...
function openCreate(){editingNodeId.value=null;editingAccountId.value=null;Object.assign(nodeForm,{parent_id:0,level:'director',name:'',credit_limit:0,permissions:catalog.value.permissions.map(item=>item.code),status:1,username:'',display_name:'',phone:'',password:''});nodeDrawer.value=true}
function openEdit(row:OrganizationNode){editingNodeId.value=row.id;editingAccountId.value=row.account_id||null;Object.assign(nodeForm,{parent_id:row.parent_id,level:row.level,name:row.name,credit_limit:Number(row.credit_limit),permissions:row.permissions.includes('*')?catalog.value.permissions.map(item=>item.code):[...row.permissions],status:row.status,username:row.username||'',display_name:row.display_name||row.name,phone:row.phone||'',password:''});nodeDrawer.value=true}
