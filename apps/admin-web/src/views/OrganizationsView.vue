...
import {
  ...
  type OrganizationCatalog, type OrganizationLevel, type OrganizationNode, type LotteryBoard,
} from '../api/admin'
...
const nodeForm=reactive({parent_id:0,level:'director' as OrganizationLevel,name:'',credit_limit:0,permissions:[] as string[],board_codes:['A'] as string[],status:1,username:'',display_name:'',phone:'',password:''})
...
function openCreate(){editingNodeId.value=null;editingAccountId.value=null;Object.assign(nodeForm,{parent_id:0,level:'director',name:'',credit_limit:0,permissions:catalog.value.permissions.map(item=>item.code),board_codes:['A'],status:1,username:'',display_name:'',phone:'',password:''});nodeDrawer.value=true}
function openEdit(row:OrganizationNode){editingNodeId.value=row.id;editingAccountId.value=row.account_id||null;Object.assign(nodeForm,{parent_id:row.parent_id,level:row.level,name:row.name,credit_limit:Number(row.credit_limit),permissions:row.permissions.includes('*')?catalog.value.permissions.map(item=>item.code):[...row.permissions],board_codes:(row as OrganizationNode & {board_codes?:string[]}).board_codes?.length? [...(row as OrganizationNode & {board_codes?:string[]}).board_codes!]:['A'],status:row.status,username:row.username||'',display_name:row.display_name||row.name,phone:row.phone||'',password:''});nodeDrawer.value=true}
function openDirectorConfig(row:OrganizationNode){if(row.level!=='director'||row.parent_id!==0)return;const children=nodes.value.filter(item=>item.parent_id===row.id);const maxRates=children.map(item=>Number(item.max_share_rate||siteMaxShareRate.value));const ownMax=Number(row.max_share_rate||siteMaxShareRate.value);Object.assign(directorConfig,{id:row.id,name:row.name,credit_limit:Number(row.credit_limit||0),max_share_rate:maxRates.length?Math.min(ownMax,...maxRates):ownMax});directorConfigDialog.value=true}
...
<template>
  <div class="organization-page">
    ...
    <el-table v-loading="loading" :data="visibleNodes" border stripe :empty-text="currentOrganization ? '当前组织暂无直属下级' : '当前站点暂无总监，请先新增总监'">
      ...
      <el-table-column label="盘口" min-width="150"><template #default="{row}"><el-tag v-for="board in (row.boards || [])" :key="board.code" size="small" class="board-tag">{{ board.name }}</el-tag></template></el-table-column>
    ...
    <el-drawer v-model="nodeDrawer" :title="editingNodeId ? `编辑${nodeForm.level === 'director' ? '总监' : '组织'}` : '新增总监'" size="560px"><el-form label-width="110px"><el-form-item label="组织层级"><el-input :model-value="catalog.levels.find(item=>item.value===nodeForm.level)?.label || nodeForm.level" disabled/></el-form-item><el-form-item :label="nodeForm.level === 'director' ? '总监名称' : '组织名称'"><el-input v-model="nodeForm.name" maxlength="120"/></el-form-item><el-form-item label="可用盘口"><el-checkbox-group v-model="nodeForm.board_codes"><el-checkbox v-for="board in boards" :key="board.code" :label="board.code">{{ board.name }}</el-checkbox></el-checkbox-group><div class="dialog-tip">下级盘口不能超过当前上级；默认启用 A 盘。会员端只显示所属代理已分配的盘口。</div></el-form-item><el-form-item label="独立分数"><el-input-number v-model="nodeForm.credit_limit" :min="0" :precision="2" controls-position="right" style="width:100%" :disabled="Boolean(editingNodeId)"/><div v-if="editingNodeId && nodeForm.level === 'director'" class="dialog-tip">已存在的总监请使用列表中的“分数占成”修改独立额度。</div></el-form-item><template v-if="!editingNodeId || editingAccountId"><el-divider content-position="left">登录账号</el-divider><el-form-item label="登录账号"><el-input v-model="nodeForm.username" maxlength="80" autocomplete="off"/></el-form-item><el-form-item label="显示名称"><el-input v-model="nodeForm.display_name" maxlength="120" placeholder="默认使用组织名称"/></el-form-item><el-form-item label="登录密码"><el-input v-model="nodeForm.password" type="password" show-password maxlength="128" autocomplete="new-password" placeholder="留空不修改；新建时至少6位，包含字母和数字"/></el-form-item><el-form-item label="手机号"><el-input v-model="nodeForm.phone" maxlength="30"/></el-form-item></template><el-form-item label="权限来源"><div class="dialog-tip">路由和按钮权限由当前站点的“路由权限”页面按层级统一配置，此处不对单独管理员设置。</div></el-form-item><el-form-item label="状态"><el-switch v-model="nodeForm.status" :active-value="1" :inactive-value="0"/></el-form-item></el-form><template #footer><el-button @click="nodeDrawer=false">取消</el-button><el-button type="primary" @click="saveNode">保存</el-button></template></el-drawer>
