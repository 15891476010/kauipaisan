...
import { ArrowLeft, Plus, Refresh, UserFilled } from '@element-plus/icons-vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import {
  createOrganization, createOrganizationAccount, deleteOrganization, deleteOrganizationAccount,
  getSiteOrganizations, updateOrganization, updateOrganizationAccount,
...
function openCreateAccount(row:OrganizationNode){editingAccountId.value=null;Object.assign(accountForm,{organization_id:row.id,username:'',display_name:'',phone:'',password:'',permissions:row.permissions.includes('*')?catalog.value.permissions.map(item=>item.code):[...row.permissions],status:1});selectedId.value=row.id;accountDrawer.value=true}
function openEditAccount(row:OrganizationAccount){editingAccountId.value=row.id;Object.assign(accountForm,{organization_id:row.organization_id,username:row.username,display_name:row.display_name,phone:row.phone||'',password:'',permissions:[...row.permissions],status:row.status});accountDrawer.value=true}
async function saveAccount(){if(!accountForm.username.trim())return ElMessage.warning('请输入管理员账号');if(accountForm.password!==''&&accountForm.password.length<6)return ElMessage.warning('登录密码至少 6 位');const payload={...accountForm,username:accountForm.username.trim(),display_name:accountForm.display_name.trim()};try{if(editingAccountId.value){await updateOrganizationAccount(editingAccountId.value,payload);ElMessage.success('管理员已更新')}else{const response=await createOrganizationAccount(accountForm.organization_id,payload);Object.assign(credential,response.data);credentialDialog.value=true}accountDrawer.value=false;await load()}catch(error){ElMessage.error(error instanceof Error?error.message:'保存失败')}}
...
function permissionText(value:string[]){if(value.includes('*'))return '全部权限';const labels=new Map(catalog.value.permissions.map(item=>[item.code,item.label]));return value.slice(0,4).map(item=>labels.get(item)||item).join('、')+(value.length>4?` 等 ${value.length} 项`:'')}
...
<template>
  <div class="organization-page">
    ...
    <el-table v-loading="loading" :data="nodes" border stripe row-key="id" highlight-current-row @current-change="selectNode">
      <el-table-column label="组织名称" min-width="240"><template #default="{row}"><div class="org-name" :style="{paddingLeft:`${Math.max(0,row.depth-1)*24}px`}"><span class="org-level-dot">{{ row.depth }}</span><div><b>{{ row.name }}</b></div></div></template></el-table-column>
      ...
      <el-table-column prop="credit_limit" label="分数额度" min-width="130" align="right" />
      <el-table-column label="功能权限" min-width="250"><template #default="{row}"><span class="permission-summary">{{ permissionText(row.permissions) }}</span></template></el-table-column>
      ...
      <el-table-column label="操作" fixed="right" width="270"><template #default="{row}"><div class="row-actions"><el-button v-if="row.next_level" link type="primary" @click.stop="openCreate(row)">新增下级</el-button><el-button link type="primary" @click.stop="openCreateAccount(row)">管理员</el-button><el-button link type="primary" @click.stop="openEdit(row)">编辑</el-button><el-button v-if="row.parent_id" link type="danger" @click.stop="removeNode(row)">删除</el-button></div></template></el-table-column>
    </el-table>
    <section v-if="selectedNode" class="account-section">
      <header><div><h3>{{ selectedNode.name }} · 管理员</h3><span>一个组织可以配置多个管理员账号，账号权限不能超过组织权限。</span></div><el-button type="primary" plain :icon="UserFilled" @click="openCreateAccount(selectedNode)">新增管理员</el-button></header>
      <el-table :data="selectedAccounts" border empty-text="当前组织暂无管理员"><el-table-column prop="username" label="账号" min-width="130"/><el-table-column prop="display_name" label="姓名" min-width="120"/><el-table-column label="权限" min-width="220"><template #default="{row}">{{ permissionText(row.permissions) }}</template></el-table-column><el-table-column label="在线" width="80"><template #default="{row}"><el-tag :type="row.online===1?'success':'info'">{{ row.online===1?'在线':'离线' }}</el-tag></template></el-table-column><el-table-column prop="last_login_at" label="最后登录" min-width="160"/><el-table-column prop="last_login_ip" label="登录 IP" min-width="180"/><el-table-column label="操作" width="130"><template #default="{row}"><el-button link type="primary" @click="openEditAccount(row)">编辑</el-button><el-button link type="danger" @click="removeAccount(row)">删除</el-button></template></el-table-column></el-table>
    </section>

    <el-drawer v-model="nodeDrawer" :title="editingNodeId?'编辑组织':'新增组织'" size="560px"><el-form label-width="100px"><el-form-item label="所属层级"><el-input :model-value="catalog.levels.find(item=>item.value===nodeForm.level)?.label||nodeForm.level" disabled/></el-form-item><el-form-item label="组织名称"><el-input v-model="nodeForm.name" maxlength="120"/></el-form-item><el-form-item label="分数额度"><el-input-number v-model="nodeForm.credit_limit" :min="0" :precision="2" controls-position="right" style="width:100%"/></el-form-item><el-form-item label="功能权限"><el-checkbox-group v-model="nodeForm.permissions" class="permission-grid"><el-checkbox v-for="item in catalog.permissions" :key="item.code" :value="item.code">{{ item.label }}</el-checkbox></el-checkbox-group></el-form-item><el-form-item label="状态"><el-switch v-model="nodeForm.status" :active-value="1" :inactive-value="0"/></el-form-item></el-form><template #footer><el-button @click="nodeDrawer=false">取消</el-button><el-button type="primary" @click="saveNode">保存</el-button></template></el-drawer>
    <el-drawer v-model="accountDrawer" :title="editingAccountId?'编辑组织管理员':'新增组织管理员'" size="560px"><el-form label-width="100px"><el-form-item label="管理员账号"><el-input v-model="accountForm.username" maxlength="80"/></el-form-item><el-form-item label="姓名"><el-input v-model="accountForm.display_name" maxlength="120"/></el-form-item><el-form-item label="手机号"><el-input v-model="accountForm.phone" maxlength="30"/></el-form-item><el-form-item :label="editingAccountId?'新密码':'登录密码'"><el-input v-model="accountForm.password" type="password" show-password :placeholder="editingAccountId?'留空不修改':'可留空，系统自动生成'"/></el-form-item><el-form-item label="账号权限"><el-checkbox-group v-model="accountForm.permissions" class="permission-grid"><el-checkbox v-for="item in catalog.permissions" :key="item.code" :value="item.code">{{ item.label }}</el-checkbox></el-checkbox-group></el-form-item><el-form-item label="状态"><el-switch v-model="accountForm.status" :active-value="1" :inactive-value="0"/></el-form-item></el-form><template #footer><el-button @click="accountDrawer=false">取消</el-button><el-button type="primary" @click="saveAccount">保存</el-button></template></el-drawer>
