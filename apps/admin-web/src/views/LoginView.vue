...
<template>
  <div class="login-page"><div class="visual"><div class="visual-copy"><span class="eyebrow">MULTI-TENANT OPERATIONS</span><h1>一个后台，管理所有代理与站点</h1><p>域名、权限、审计与业务数据统一隔离，清楚掌握每一个租户的运行状态。</p></div></div><div class="login-panel"><form class="login-card" @submit.prevent="submit"><div class="logo">K</div><h2>欢迎回来</h2><p>登录快排 SaaS 管理平台</p><el-input v-model="form.username" size="large" placeholder="管理账号" autocomplete="username" /><el-input v-model="form.password" size="large" type="password" show-password placeholder="登录密码" autocomplete="current-password" @keyup.enter="submit" /><el-button native-type="submit" type="primary" size="large" :loading="loading">登录管理平台</el-button></form></div></div>
</template>
<style scoped>
