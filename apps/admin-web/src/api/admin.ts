...
export type BetDetail = { id: number; row_key?: string; lottery: string; number_text?: string; number_count?: number; hundreds: string; tens: string; units: string; amount: string; category?: string; play_type?: string; source_text?: string; win_amount?: string; result_status?: string; draw_numbers?: string }
...
export const updateBetDetail = (id: number, payload: Pick<BetDetail, 'hundreds' | 'tens' | 'units' | 'amount'>) => http.put<never, Envelope<null>>(`/admin/bet-details/${id}`, payload)
