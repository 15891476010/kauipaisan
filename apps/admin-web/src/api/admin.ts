...
export type BetDetail = { id: number; row_key?: string; number_index?: number; lottery: string; number_text?: string; number_count?: number; hundreds: string; tens: string; units: string; amount: string; odds?: string; category?: string; play_type?: string; source_text?: string; win_amount?: string; result_status?: 'pending'|'won'|'partial'|'unwon'|string; draw_numbers?: string }
...
export const updateBetDetail = (id: number, payload: { number_index?: number; number_text: string; amount: string; play_type?: string; source_text?: string }) => http.put<never, Envelope<null>>(`/admin/bet-details/${id}`, payload)
...
export type MasterBetDetail = { detail_id: number; lottery: string; number_text: string; play_type: string; amount: string }
export type MasterBetRecord = { record_id: number; id: number; site_id: number; user_id: number; username: string; site_name: string; issue_no: string; submission_id?: number; amount: string; bet_count: number; source_text?: string; formatted_text?: string; details: MasterBetDetail[] }
export type MasterBatchPreview = { record_ids: number[]; changed_count: number; skipped_count: number; changes: Array<{ record_id: number; detail_id: number; issue_no: string; old_number: string; new_number: string; old_play: string; new_play: string; old_amount: string; new_amount: string }>; skipped: Array<{ record_id: number; detail_id: number; reason: string }> }
export const getMasterBetOptions = () => http.get<never, Envelope<{ records: MasterBetRecord[] }>>('/admin/bet-records/batch-options')
export const previewMasterBetBatch = (payload: { record_ids: number[]; operation: 'replace_number'|'replace_play'|'set_amount'; payload: Record<string, unknown> }) => http.post<never, Envelope<MasterBatchPreview>>('/admin/bet-records/batch-preview', payload)
export const applyMasterBetBatch = (payload: { record_ids: number[]; operation: 'replace_number'|'replace_play'|'set_amount'; payload: Record<string, unknown> }) => http.post<never, Envelope<{ changed: number }>>('/admin/bet-records/batch-apply', payload)
...
export type OrganizationResponse = { site: { id: number; name: string; credit_limit?: string; available_balance?: string; director_allocated_score?: string; director_count?: number }; nodes: OrganizationNode[]; members?: unknown[]; accounts: OrganizationAccount[]; boards?: LotteryBoard[]; catalog: OrganizationCatalog; site_max_share_rate?: string }
...
export type SiteBettingControl = { cutoff_enabled: number; cutoff_time: string | null; mask_enabled: number; refund_enabled: number; timing_rules?: SiteTimingRule[] }
...
export interface Lottery { id: number; name: string; code: string; unit_stake: string; source_type: 'official'|'system'; system_interval_seconds: number; system_issue_mode: 'auto'|'manual'; system_initial_issue: string | null; odds_source_lottery_id?: number | null; status: number; sort: number; site_ids: number[]; cutoff_enabled: number; cutoff_time: string | null; mask_enabled: number; refund_enabled: number }
...
export type LotterySourceSettings = { source_type: 'official'|'system'; system_interval_seconds: number; system_issue_mode: 'auto'|'manual'; system_initial_issue: string | null; odds_source_lottery_id?: number | null }
export const createLottery = (payload: { name: string; code: string; unit_stake: number; status: number; sort: number } & Partial<LotteryBettingControls> & Partial<LotterySourceSettings>) => http.post<never, Envelope<{ id: number }>>('/admin/lotteries', payload)
export const updateLottery = (id: number, payload: Partial<{ name: string; code: string; unit_stake: number; status: number; sort: number } & LotteryBettingControls & LotterySourceSettings>) => http.put<never, Envelope<null>>(`/admin/lotteries/${id}`, payload)
...
export type LotteryHistory = { id: number; code: string; draw_day: string; one: number; two: number; three: number; numbers: string; open_time: string; next_code?: string; is_opened: number }
...
export const updateLotteryHistory = (id: number, payload: { numbers: string }) => http.put<never, Envelope<{ id: number; numbers: string }>>(`/admin/lottery-history/${id}`, payload)
export type LotteryBoard = { id?: number; code: string; name: string; status?: number; sort?: number }
export const listLotteryBoards = () => http.get<never, Envelope<{ list: LotteryBoard[] }>>('/admin/lottery-boards')
export const createLotteryBoard = (payload: { code: string; name: string; status?: number; sort?: number }) => http.post<never, Envelope<LotteryBoard>>('/admin/lottery-boards', payload)
export const updateLotteryBoard = (id: number, payload: Partial<LotteryBoard>) => http.put<never, Envelope<null>>(`/admin/lottery-boards/${id}`, payload)
export const copyLotteryOdds = (id: number, source_lottery_id: number, replace = false, board_code = 'A') => http.post<never, Envelope<null>>(`/admin/lotteries/${id}/copy-odds`, { source_lottery_id, replace: replace ? 1 : 0, board_code })
...
export const listLotteryOdds = (id: number, params?: { page?: number; page_size?: number; board_code?: string }) => http.get<never, Envelope<{ categories: LotteryOddsCategory[]; total: number; category_total: number; page: number; page_size: number; board_code: string; boards: LotteryBoard[] }>>(`/admin/lottery-odds/${id}`, { params })
