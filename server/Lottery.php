...
final class Lottery
{
    ...
    private function normalizeBoardCode(mixed $value): string
    {
        $value=strtoupper(trim((string)$value));
        return preg_match('/^[A-Z][A-Z0-9_-]{0,7}$/',$value)===1?$value:'A';
    }
    private function boardList(int $tenantId): array
    {
        $rows=Db::name('lottery_boards')->where('tenant_id',$tenantId)->where('status',1)->order('sort asc')->order('id asc')->select()->toArray();
        if ($rows===[]) return [['code'=>'A','name'=>'A盘','sort'=>1]];
        return array_map(static fn(array $row): array=>['code'=>(string)$row['code'],'name'=>(string)$row['name'],'sort'=>(int)$row['sort']],$rows);
    }
    ...
    public function create(Request $request): \think\response\Json
    {
        ...
        if ($templateId>0) $this->copyOddsFromLottery($templateId,$id,false,'A','A');
    ...
    public function update(Request $request): \think\response\Json
    {
        ...
        if ($templateChanged && (int)($data['odds_source_lottery_id']??0)>0) $this->copyOddsFromLottery((int)$data['odds_source_lottery_id'],$id,true,'A','A');
    ...
    public function boards(Request $request): \think\response\Json
    {
        $session=$this->session($request,'admin');
        return $this->reply(['list'=>$this->boardList((int)($session['tenant_id']??1))]);
    }
    public function createBoard(Request $request): \think\response\Json
    {
        $session=$this->session($request,'admin');$tenantId=(int)($session['tenant_id']??1);$data=$request->post();
        $code=strtoupper(trim((string)($data['code']??'')));$name=trim((string)($data['name']??($code.'盘')));
        if(!preg_match('/^[A-Z][A-Z0-9_]{0,7}$/',$code))throw new \InvalidArgumentException('盘口编码只能使用字母、数字和下划线');
        if($name==='')throw new \InvalidArgumentException('请输入盘口名称');
        if(Db::name('lottery_boards')->where('tenant_id',$tenantId)->where('code',$code)->find())throw new \InvalidArgumentException('盘口编码已存在');
        $now=date('Y-m-d H:i:s');$id=(int)Db::name('lottery_boards')->insertGetId(['tenant_id'=>$tenantId,'code'=>$code,'name'=>$name,'status'=>(int)($data['status']??1)===1?1:0,'sort'=>(int)($data['sort']??0),'settings'=>json_encode((array)($data['settings']??[]),JSON_UNESCAPED_UNICODE),'created_at'=>$now,'updated_at'=>$now]);
        return $this->reply(['id'=>$id,'code'=>$code,'name'=>$name],'盘口创建成功');
    }
    public function updateBoard(Request $request,int $id): \think\response\Json
    {
        $session=$this->session($request,'admin');$tenantId=(int)($session['tenant_id']??1);$row=Db::name('lottery_boards')->where('id',$id)->where('tenant_id',$tenantId)->find();if(!$row)throw new \InvalidArgumentException('盘口不存在');$data=$request->put();$update=['updated_at'=>date('Y-m-d H:i:s')];
        if(array_key_exists('name',$data)){ $name=trim((string)$data['name']);if($name==='')throw new \InvalidArgumentException('请输入盘口名称');$update['name']=$name; }
        if(array_key_exists('status',$data))$update['status']=(int)$data['status']===1?1:0;if(array_key_exists('sort',$data))$update['sort']=(int)$data['sort'];
        if($row['code']==='A')$update['status']=1;
        Db::name('lottery_boards')->where('id',$id)->update($update);return $this->reply(null,'盘口已更新');
    }
    ...
    private function siteList(int $siteId, int $tenantId=1, int $userId=0): array
    {
        ...
        $rows=Db::name('lotteries')->alias('l')
            ...
            ->field('l.id,l.name,l.code,l.sort,l.unit_stake,l.source_type,l.system_interval_seconds,l.system_issue_mode,l.system_initial_issue,l.odds_source_lottery_id,l.default_board_code,l.cutoff_enabled,l.cutoff_time,l.mask_enabled,l.refund_enabled')
            ->order('l.sort asc')
            ->order('l.id asc')
            ->select()->toArray();
        $permissionMap=[];
        ...
        $allowedBoards=['A'];
        if ($userId>0) {
            $settings=Db::name('site_users')->alias('u')->join('organization_nodes n','n.id=u.organization_id')->where('u.id',$userId)->value('n.settings');
            $decoded=is_string($settings)?json_decode($settings,true):$settings;
            if(is_array($decoded)&&is_array($decoded['board_codes']??null)&&$decoded['board_codes']!==[])$allowedBoards=array_values(array_map('strval',$decoded['board_codes']));
        }
        foreach ($rows as &$row) {
            ...
            $row['boards']=array_values(array_filter($this->boardList($tenantId),static fn(array $board): bool=>in_array((string)$board['code'],$allowedBoards,true)));
            $row['board_code']=$this->normalizeBoardCode($row['default_board_code']??'A');
            if(!in_array($row['board_code'],$allowedBoards,true))$row['board_code']=$allowedBoards[0]??'A';
        }
        unset($row);
        return $rows;
    }
    ...
    public function copyOdds(Request $request, int $id): \think\response\Json
    {
        ...
        $boardCode=$this->normalizeBoardCode($data['board_code']??$data['board']??'A');
        $this->copyOddsFromLottery($sourceId,$id,$replace,$boardCode,$boardCode); Db::name('lotteries')->where('id',$id)->update(['odds_source_lottery_id'=>$sourceId,'updated_at'=>date('Y-m-d H:i:s')]);
    ...
    public function odds(Request $request): \think\response\Json
    {
        ...
        $boardCode=$this->normalizeBoardCode($request->param('board_code',$request->param('board','A')));
        $categoryQuery=Db::name('lottery_odds_categories')->where('lottery_id',$lotteryId)->where('board_code',$boardCode)->whereNull('deleted_at');
        ...
        foreach ($categories as &$category) {
            $category['children']=(int)$category['is_playable']===1 ? [] : Db::name('lottery_odds')->where('lottery_id',$lotteryId)->where('board_code',$boardCode)->where('category_id',(int)$category['id'])->whereNull('deleted_at')->order('sort asc')->order('id asc')->select()->toArray();
        }
        unset($category);
        $total=(int)Db::name('lottery_odds')->where('lottery_id',$lotteryId)->where('board_code',$boardCode)->whereNull('deleted_at')->count()
            +(int)Db::name('lottery_odds_categories')->where('lottery_id',$lotteryId)->where('board_code',$boardCode)->where('is_playable',1)->whereNull('deleted_at')->count();
        return $this->reply(['categories'=>$categories,'total'=>$total,'category_total'=>$categoryTotal,'page'=>$page,'page_size'=>$pageSize,'board_code'=>$boardCode,'boards'=>$this->boardList((int)($session['tenant_id']??1))]);
    }
    public function createOddsCategory(Request $request): \think\response\Json
    {
        ...
        $lottery=$this->catalog()->where('id',$lotteryId)->find(); if(!$lottery) throw new \InvalidArgumentException('彩票不存在');
        $now=date('Y-m-d H:i:s'); $row=['tenant_id'=>(int)$lottery['tenant_id'],'lottery_id'=>$lotteryId,'board_code'=>$this->normalizeBoardCode($data['board_code']??$data['board']??'A'),'name'=>$name,'is_playable'=>(int)($data['is_playable']??0)===1?1:0,'status'=>(int)($data['status']??1),'sort'=>(int)($data['sort']??0),'created_at'=>$now,'updated_at'=>$now]; if($row['is_playable']) $row=array_merge($row,$this->nullableOddsNumbers($data)); $id=(int)Db::name('lottery_odds_categories')->insertGetId($row);
    ...
    public function createOdds(Request $request): \think\response\Json
    {
        ...
        $numeric=$this->nullableOddsNumbers($data); $now=date('Y-m-d H:i:s'); $id=Db::name('lottery_odds')->insertGetId(array_merge(['tenant_id'=>(int)$category['tenant_id'],'lottery_id'=>$lotteryId,'board_code'=>$this->normalizeBoardCode($data['board_code']??$data['board']??$category['board_code']??'A'),'category_id'=>$categoryId,'category'=>(string)$category['name'],'name'=>$name,'status'=>(int)($data['status']??1),'sort'=>(int)($data['sort']??0),'created_at'=>$now,'updated_at'=>$now],$numeric)); return $this->reply(['id'=>$id],'玩法赔率已创建');
    }
    ...
    private function copyOddsFromLottery(int $sourceId,int $targetId,bool $replace=false,string $sourceBoard='A',string $targetBoard='A'): void
    {
        $sourceBoard=$this->normalizeBoardCode($sourceBoard);$targetBoard=$this->normalizeBoardCode($targetBoard);
        $existing=(int)Db::name('lottery_odds_categories')->where('lottery_id',$targetId)->where('board_code',$targetBoard)->whereNull('deleted_at')->count();
        if ($existing>0 && !$replace) throw new \InvalidArgumentException('当前彩种已有赔率，请先确认替换或清空后再复制');
        $sourceCategories=Db::name('lottery_odds_categories')->where('lottery_id',$sourceId)->where('board_code',$sourceBoard)->whereNull('deleted_at')->order('sort asc')->order('id asc')->select()->toArray();
        if ($sourceCategories===[]) throw new \InvalidArgumentException('来源彩种暂无可复制的赔率');
        Db::transaction(function() use($sourceId,$targetId,$sourceCategories,$replace,$sourceBoard,$targetBoard): void {
            if ($replace) {
                ...
                Db::name('user_lottery_odds')->where('lottery_id',$targetId)->where('board_code',$targetBoard)->delete();
                Db::name('lottery_odds')->where('lottery_id',$targetId)->where('board_code',$targetBoard)->delete();
                Db::name('lottery_odds_categories')->where('lottery_id',$targetId)->where('board_code',$targetBoard)->delete();
            }
            $now=date('Y-m-d H:i:s');
            foreach ($sourceCategories as $sourceCategory) {
                $categoryData=['tenant_id'=>(int)$sourceCategory['tenant_id'],'lottery_id'=>$targetId,'board_code'=>$targetBoard,'name'=>(string)$sourceCategory['name'],'is_playable'=>(int)$sourceCategory['is_playable'],'min_bet'=>$sourceCategory['min_bet'],'odds_limit'=>$sourceCategory['odds_limit'],'single_bet_limit'=>$sourceCategory['single_bet_limit'],'single_item_limit'=>$sourceCategory['single_item_limit'],'odds'=>$sourceCategory['odds'],'offline_rebate'=>'0.0000','status'=>(int)$sourceCategory['status'],'sort'=>(int)$sourceCategory['sort'],'created_at'=>$now,'updated_at'=>$now];
                $newCategoryId=(int)Db::name('lottery_odds_categories')->insertGetId($categoryData);
                $plays=Db::name('lottery_odds')->where('lottery_id',$sourceId)->where('board_code',$sourceBoard)->where('category_id',(int)$sourceCategory['id'])->whereNull('deleted_at')->select()->toArray();
                foreach ($plays as $play) Db::name('lottery_odds')->insert(['tenant_id'=>(int)$play['tenant_id'],'lottery_id'=>$targetId,'board_code'=>$targetBoard,'category_id'=>$newCategoryId,'category'=>(string)$sourceCategory['name'],'name'=>(string)$play['name'],'min_bet'=>$play['min_bet'],'odds_limit'=>$play['odds_limit'],'single_bet_limit'=>$play['single_bet_limit'],'single_item_limit'=>$play['single_item_limit'],'odds'=>$play['odds'],'offline_rebate'=>'0.0000','status'=>(int)$play['status'],'sort'=>(int)$play['sort'],'created_at'=>$now,'updated_at'=>$now]);
            }
        });
    }
}
