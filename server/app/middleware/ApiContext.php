...
final class ApiContext
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (str_starts_with($path, 'api/v1/agent/') && !str_contains($path, '/auth/') && !str_ends_with($path, '/agreement') && !str_ends_with($path, '/announcement') && !str_ends_with($path, '/lotteries')) {
            ...
            if (is_array($agentSession) && !empty($agentSession['is_subaccount'])) {
                ...
                if ($required === '__owner__' || ($required !== '' && !in_array($required, $permissions, true))) return $this->cors(json(['code'=>403,'message'=>'当前子账号没有此功能权限','data'=>null,'request_id'=>bin2hex(random_bytes(8))],403));
            }
            if (($agentSession['account_table']??'')==='organization_accounts') {
                ...
                if($refreshed!==$agentSession&&$token!=='')Cache::set('token:'.$token,$refreshed,(int)env('TOKEN_TTL',7200));$agentSession=$refreshed;
                $required=$this->agentPermission($path,$request);
                if($required!==''&&$required!=='__owner__'&&!in_array('*',$permissions,true)&&!in_array($required,$permissions,true))return $this->cors(json(['code'=>403,'message'=>'上级未分配此功能权限','data'=>null,'request_id'=>bin2hex(random_bytes(8))],403));
            }
        }
        $authorization = (string)$request->header('authorization');
        $token = trim(str_ireplace('Bearer ', '', $authorization));
    ...
    {
        if (str_contains($path,'/subaccounts')) return '__owner__';
        if (str_contains($path,'/organizations') || str_contains($path,'/organization-accounts') || str_contains($path,'/profit-shares')) return 'organization.manage';
        if (str_contains($path,'/ledger/issues') || str_contains($path,'/interceptions/issues') || str_contains($path,'/interceptions/categories')) return '';
        if (str_contains($path,'/members')) return 'subordinates';
