...
final class QuickEntryRules
{
    ...
    private const PATTERNS = [
        ...
        // A play can appear before its number selection, including a single
        // play (“组三023456各1”) or several plays sharing one amount.
        'multi_play_prefix_amount' => '/^\s*(?:福体|福|体)?\s*((?:(?:组三|组六|复式)\s*){1,3})([0-9]{2,10})\s*(?:各|每)\s*(\d+(?:\.\d+)?)\s*(元|米|块|角|毛|倍)?\s*$/u',
    ...
    public function oddsIdentity(string $source): ?array
    {
        ...
        // 粘边赖的旧录入格式把“码数”放在玩法前面，例如“345三码组三”。
        // 该格式必须仍然映射到赖赔率目录，不能落到普通组三/组六赔率。
        if (preg_match('/([一二两三四五六七])码\s*(组三|组六)/u', $source, $match)) {
            $family = $match[2].'赖';
            $identity = $this->catalogPlay($family, $match[1]);
            if ($identity !== null) return ['category'=>$identity['category'], 'name'=>$identity['name'], 'direct'=>false];
        }
