...
final class ScoreLedger
{
    ...
    public function dashboard(Request $request): \think\response\Json
    {
        ...
        $siteCreditRows=Db::name('site_credit_accounts')->where('tenant_id',$tenantId)->field('site_id,total_score,balance')->select()->toArray();
        ...
        $today=date('Y-m-d');
        $userRows=Db::name('site_users')->where('tenant_id',$tenantId)->whereNull('deleted_at')->field("site_id,COUNT(*) user_count,COALESCE(SUM(balance+credit_balance-CASE WHEN used_balance_date='{$today}' THEN used_balance ELSE 0 END),0) available_score,COALESCE(SUM(CASE WHEN used_balance_date='{$today}' THEN used_balance ELSE 0 END),0) locked_score")->group('site_id')->select()->toArray();
        $rootMap=$this->rowsByKey($rootRows,'site_id');$siteCreditMap=$this->rowsByKey($siteCreditRows,'site_id');$organizationMap=$this->rowsByKey($organizationRows,'site_id');$userMap=$this->rowsByKey($userRows,'site_id');$sites=[];
        foreach(Db::name('sites')->where('tenant_id',$tenantId)->whereNull('deleted_at')->order('id asc')->field('id,name,status')->select()->toArray()as$site){$siteId=(int)$site['id'];$root=$rootMap[$siteId]??[];$siteCredit=$siteCreditMap[$siteId]??[];$organizations=$organizationMap[$siteId]??[];$users=$userMap[$siteId]??[];$siteAvailable=(float)($siteCredit['balance']??0);$organizationAvailable=(float)($organizations['available_score']??0);$userAvailable=(float)($users['available_score']??0);$locked=(float)($users['locked_score']??0);$sites[]=['site_id'=>$siteId,'site_name'=>(string)$site['name'],'status'=>(int)$site['status'],'allocated_score'=>$this->money($siteCredit['total_score']??0),'site_available'=>$this->money($siteAvailable),'director_allocated_score'=>$this->money($root['allocated_score']??0),'organization_available'=>$this->money($organizationAvailable),'user_available'=>$this->money($userAvailable),'user_locked'=>$this->money($locked),'circulating_score'=>$this->money($siteAvailable+$organizationAvailable+$userAvailable+$locked),'organization_count'=>(int)($organizations['organization_count']??0),'user_count'=>(int)($users['user_count']??0)];}
        ...
        $accounted=(float)$overview['available_score']+(float)$overview['site_available']+(float)$overview['organization_available']+(float)$overview['user_available']+(float)$overview['user_locked'];
    ...
    private function overviewData(int $tenantId): array
    {
        ...
        $siteAvailable=(float)Db::name('site_credit_accounts')->where('tenant_id',$tenantId)->sum('balance');
        $today=date('Y-m-d');$userLocked=(float)Db::name('site_users')->where('tenant_id',$tenantId)->whereNull('deleted_at')->where('used_balance_date',$today)->sum('used_balance');$userTotal=(float)Db::name('site_users')->where('tenant_id',$tenantId)->whereNull('deleted_at')->sum('balance')+(float)Db::name('site_users')->where('tenant_id',$tenantId)->whereNull('deleted_at')->sum('credit_balance')-$userLocked;
        $allocated=(float)Db::name('site_credit_accounts')->where('tenant_id',$tenantId)->sum('total_score');
        return['total_score'=>$this->money($account['total_score']),'available_score'=>$this->money($account['balance']),'allocated_score'=>$this->money($allocated),'site_available'=>$this->money($siteAvailable),'organization_available'=>$this->money($organizationTotal),'user_available'=>$this->money($userTotal),'user_locked'=>$this->money($userLocked)];
    }
    ...
    private function appendNames(array &$row): void
    {
        ...
        $row['account_name']=match($type){'platform'=>'总平台','site'=>(string)(Db::name('sites')->where('id',(int)$row['site_id'])->value('name')?:'站点账户'),'organization'=>(string)(Db::name('organization_nodes')->where('id',$id)->value('name')?:''),'user'=>(string)(Db::name('site_users')->where('id',$id)->value('username')?:''),default=>''};
        $counterType=(string)($row['counterparty_account_type']??'');$counterId=(int)($row['counterparty_account_id']??0);$row['counterparty_name']=match($counterType){'platform'=>'总平台','site'=>(string)(Db::name('sites')->where('id',$counterId)->value('name')?:'站点账户'),'organization'=>(string)(Db::name('organization_nodes')->where('id',$counterId)->value('name')?:''),'user'=>(string)(Db::name('site_users')->where('id',$counterId)->value('username')?:''),default=>''};
