...
final class AgentInterception
{
    ...
    {
        $session = $this->session($request); $lottery = $this->lottery($session, trim((string)$request->param('lottery', ''))); $boardCode=$this->boardCode($request,$session);
        $rows = Db::name('lottery_odds')->where('lottery_id', (int)$lottery['id'])->where('board_code',$boardCode)->where('status', 1)->whereNull('deleted_at')
            ->field('id,category,name,odds,sort')->order('sort asc')->order('id asc')->select()->toArray();
        foreach (Db::name('lottery_odds_categories')->where('lottery_id', (int)$lottery['id'])->where('board_code',$boardCode)->where('is_playable', 1)->where('status', 1)->whereNull('deleted_at')->field('id,name,odds,sort')->select()->toArray() as $row) {
        ...
        return $this->reply(['list' => $rows,'board_code'=>$boardCode]);
    }
    ...
    public function index(Request $request): \think\response\Json
    {
        ...
        $query = Db::name('agent_interceptions')->alias('i')
            ...
            ->where('i.site_id', (int)$session['site_id'])->where('i.lottery_id', (int)$lottery['id'])->where('i.board_code',$boardCode)
        ...
        $rows = $query->field('i.id,i.lottery_odds_id,i.bet_record_id,i.issue_no,i.number_key,i.bet_amount,i.share_rate,i.requested_amount,i.intercepted_amount,i.allocation_status,i.created_at,u.username,u.display_name,d.amount AS detail_amount,d.odds,d.rebate,d.win_amount,o.category,o.name AS odds_name')
        ...
        $directNames = $directIds === [] ? [] : Db::name('lottery_odds_categories')->whereIn('id', array_values(array_unique($directIds)))->where('board_code',$boardCode)->column('name', 'id');
        ...
        return $this->reply(['list' => $rows, 'total' => count($rows), 'board_code'=>$boardCode, 'summary' => $this->summary($rows)]);
    }

    public function plate(Request $request): \think\response\Json
    {
        $session = $this->session($request); $siteId = (int)$session['site_id'];
        $lottery = $this->lottery($session, trim((string)$request->param('lottery', ''))); $lotteryId = (int)$lottery['id']; $boardCode=$this->boardCode($request,$session);
        ...
        $configuration=(new \app\service\InterceptionAllocator())->agentConfiguration((int)$session['tenant_id'],$siteId,(int)$node['id'],$lotteryId,false,$boardCode);
        ...
        $usageRows=Db::name('interception_capacity_usage')->where('tenant_id',(int)$session['tenant_id'])->where('scope_type','organization')->where('scope_id',(int)$node['id'])
            ->where('lottery_id',$lotteryId)->where('board_code',$boardCode)->where('issue_no',$issue)->where('used_amount','>',0)->where('number_key','__PLAY_TOTAL__')->field('lottery_odds_id,used_amount')->order('lottery_odds_id asc')->select()->toArray();
        ...
        $odds = Db::name('lottery_odds')->where('lottery_id', $lotteryId)->where('board_code',$boardCode)->where('status', 1)->whereNull('deleted_at')->field('id,category,name,odds,sort')->order('sort asc')->order('id asc')->select()->toArray();
        foreach (Db::name('lottery_odds_categories')->where('lottery_id', $lotteryId)->where('board_code',$boardCode)->where('is_playable', 1)->where('status', 1)->whereNull('deleted_at')->field('id,name,odds,sort')->select()->toArray() as $row) {
        ...
        return $this->reply(['issue_no' => $issue, 'board_code'=>$boardCode,'capacity_scope'=>'organization','organization_id'=>(int)$node['id'],'configuration_source'=>$configuration['source'],'groups' => $list]);
    }
