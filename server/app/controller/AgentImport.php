...
final class AgentImport
{
    ...
    private function call(string $base,string $ak,string $a,string $m,array $payload=[]): array { $obj=array_merge(['a'=>$a,'m'=>$m,'ak'=>$ak],$payload); $url=rtrim($base,'/').'/ag/';$curlOptions=[CURLOPT_RETURNTRANSFER=>true,CURLOPT_POST=>true,CURLOPT_TIMEOUT=>12,CURLOPT_CONNECTTIMEOUT=>5,CURLOPT_HTTPHEADER=>['Content-Type: application/x-www-form-urlencoded'],CURLOPT_POSTFIELDS=>'k='.rawurlencode(base64_encode(json_encode($obj,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)))];$parts=parse_url($url);$host=(string)($parts['host']??'');$scheme=(string)($parts['scheme']??'https');$port=(int)($parts['port']??($scheme==='http'?80:443));if($host!==''){if(!array_key_exists($host,self::$dnsCache)){ $resolved=gethostbyname($host); self::$dnsCache[$host]=($resolved!==$host?$resolved:''); }if(self::$dnsCache[$host]!=='')$curlOptions[CURLOPT_RESOLVE]=[$host.':'.$port.':'.self::$dnsCache[$host]];}$ch=curl_init($url);curl_setopt_array($ch,$curlOptions);$body=curl_exec($ch);$err=curl_error($ch);$status=(int)curl_getinfo($ch,CURLINFO_HTTP_CODE);curl_close($ch);if($body===false)throw new \RuntimeException($err?:'请求失败');return ['http_status'=>$status,'request'=>$obj,'response'=>$this->decode((string)$body)]; }
    ...
    private function snapshotAccountTree(string $base,string $ak,int $batchId,array $rootRows): int
    {
        ...
        while($queue!==[]&&$depth<12){$next=[];foreach($queue as $parent){if(isset($seen[$parent]))continue;$seen[$parent]=true;
            try{$page=1;$pages=1;do{$result=$this->call($base,$ak,'ag.ac','gal',['pn'=>$page,'ps'=>100,'lt'=>4,'pi'=>$parent]);$response=$result['response'];$data=$response['data']??[];$rows=(array)($data['al']??$data['list']??[]);
                ...
                Db::name('agent_import_records')->insert(['batch_id'=>$batchId,'entity_type'=>'sync_log','external_id'=>$parent,'local_id'=>null,'action'=>'progress','payload'=>json_encode(['level'=>'info','message'=>'已获取下级账号列表','context'=>['parent_external_id'=>$parent,'rows'=>count($rows)]],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'created_at'=>date('Y-m-d H:i:s')]);
            ...
            }catch(\Throwable $e){Db::name('agent_import_records')->insert(['batch_id'=>$batchId,'entity_type'=>'account_tree','external_id'=>$parent,'local_id'=>null,'action'=>'error','payload'=>json_encode(['parent_external_id'=>$parent,'error'=>$e->getMessage()],JSON_UNESCAPED_UNICODE),'created_at'=>date('Y-m-d H:i:s')]);Db::name('agent_import_records')->insert(['batch_id'=>$batchId,'entity_type'=>'sync_log','external_id'=>$parent,'local_id'=>null,'action'=>'progress','payload'=>json_encode(['level'=>'warning','message'=>'下级账号列表获取失败，已跳过并继续','context'=>['parent_external_id'=>$parent,'error'=>$e->getMessage()]],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'created_at'=>date('Y-m-d H:i:s')]);}
        }$queue=$next;$depth++;}
        return $total;
    }
    ...
    public function createBatchResolved(Request $r): \think\response\Json
    {
        ...
        $id=(int)Db::name('agent_import_batches')->insertGetId(['tenant_id'=>(int)$s['tenant_id'],'site_id'=>$site,'profile_id'=>(int)$profile['id'],'target_organization_id'=>(int)($in['target_organization_id']??0),'from_date'=>$from,'to_date'=>$to,'types'=>json_encode(['report_overview','accounts','account_tree','sync_log'],JSON_UNESCAPED_UNICODE),'status'=>'running','started_at'=>date('Y-m-d H:i:s'),'created_at'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s')]);
        try {
            ...
            // The accounting import intentionally pulls only the total
            // overview and the account tree. Other reports are derived from
            // the inserted local bet records.
            $specs=[['report_overview','ag.tz','gblr',['lt'=>4,'dnf'=>$range['from'],'dnt'=>$range['to'],'pn'=>1,'ps'=>40]],['accounts','ag.ac','gal',['pn'=>1,'ps'=>100,'lt'=>4]]];
            $counts=[]; foreach($specs as [$type,$a,$m,$params]){try{
                if($type==='report_overview'){
                    $page=1; $totalPages=1; $count=0;
                    do { $params['pn']=$page; $result=null; $pageError=null; for($attempt=1;$attempt<=3&&$result===null;$attempt++){try{$result=$this->call($profile['base_url'],$ak,$a,$m,$params);}catch(\Throwable $e){$pageError=$e; usleep(250000);}} if($result===null){if($pageError)Db::name('agent_import_records')->insert(['batch_id'=>$id,'entity_type'=>$type,'action'=>'page_error','payload'=>json_encode(['page'=>$page,'error'=>$pageError->getMessage()],JSON_UNESCAPED_UNICODE),'created_at'=>date('Y-m-d H:i:s')]); break;} $response=$result['response']; $data=$response['data']??[]; $rows=is_array($data)?(array)($data['rl']??$data['list']??[]):[]; $count+=count($rows); $total=(int)($data['tc']??$data['total']??0); $totalPages=$total>0?(int)ceil($total/40):($rows!==[]?$page:0); Db::name('agent_import_records')->insert(['batch_id'=>$id,'entity_type'=>$type,'external_id'=>null,'local_id'=>null,'action'=>'snapshot','payload'=>json_encode(['request'=>$result['request'],'response'=>$response],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'created_at'=>date('Y-m-d H:i:s')]); $page++; } while($page<=$totalPages&&$page<=200);
