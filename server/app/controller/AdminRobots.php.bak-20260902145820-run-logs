...
final class AdminRobots
{
    ...
    public function clearLatest(Request $request,int $id): \think\response\Json
    {
        ...
        try{$result=Db::transaction(function()use($robot):array{
            ...
            if($cutoff===''){$this->restoreRobotBaseline($robot);Db::name('robot_accounts')->where('id',(int)$robot['id'])->update(['last_bet_at'=>null,'next_run_at'=>(string)$lockedRobot['status']==='running'?$lockedRobot['start_at']:null,'updated_at'=>date('Y-m-d H:i:s')]);return ['deleted'=>0,'cutoff'=>null];}
            ...
            if($records===[]){$this->restoreRobotBaseline($robot);Db::name('robot_accounts')->where('id',(int)$robot['id'])->update(['last_bet_at'=>null,'next_run_at'=>(string)$lockedRobot['status']==='running'?$lockedRobot['start_at']:null,'updated_at'=>date('Y-m-d H:i:s')]);return ['deleted'=>0,'cutoff'=>$cutoff];}
