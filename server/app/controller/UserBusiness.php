    ...
    public function bills(Request $request): \think\response\Json
    {
        ...
        if ($lotteryFilter!=='') {
            $detailQuery=Db::name('bet_details')->alias('d')->leftJoin('user_stop_drops s','s.bet_detail_id=d.id')
                ->where('d.site_id',$s['site_id'])->where('d.user_id',$s['user_id'])
                ->where('d.status','<>','refunded')->where('s.lottery',$lotteryFilter);
...
/u',$groupText)?:[],static fn(string $value):bool=>trim($value)!==''));
    ...
    public function quickPlace(Request $request): \think\response\Json
    {
        ...
        $providerResult=null;
        // Robot scheduler tickets are generated by the platform itself, so
        // the local parser may replace the rate-limited remote recognition
        // service.  The flag is honored only for the protected worker
        // session; member submissions always use the provider.
        $robotLocal=(bool)($s['robot_scheduler']??false) && (string)$request->post('robot_local_parse','')==='1';
        if($robotLocal){
            $lines=(new QuickEntryParser())->parse($text,$lottery);
        }else{
            try {
