...
final class AgentLedger
{
    ...
    {
        if($issues===[])return[];$query=Db::name('bet_details')->alias('d')->join('bet_records r','r.id=d.bet_record_id')->join('site_users u','u.id=d.user_id')->leftJoin('user_stop_drops s','s.bet_detail_id=d.id')->where('d.site_id',(int)$session['site_id'])->whereIn('d.issue_no',$issues)->where('s.lottery',$lottery)->where('r.status','<>','refunded');OrganizationHierarchy::applyUserScope($query,$session,'d.user_id');$rows=$query->field('d.id,d.user_id,u.username,d.issue_no,d.category,d.number_text,d.amount,d.win_amount,d.rebate,s.drop_odds')->select()->toArray();if($rows===[])return[];
        ...
        $waterRows=Db::name('organization_water_ledger')->where('tenant_id',(int)($session['tenant_id']??0))->where('site_id',(int)$session['site_id'])->whereIn('related_bet_detail_id',$ids)->where('source_type','water_profit')->field('related_bet_detail_id,SUM(amount) amount')->group('related_bet_detail_id')->select()->toArray();$waterMap=[];foreach($waterRows as $waterRow)$waterMap[(int)$waterRow['related_bet_detail_id']]=(float)$waterRow['amount'];
        foreach($rows as &$row){$amount=(float)$row['amount'];$win=(float)$row['win_amount'];$rebate=(float)$row['rebate'];$offlineWater=array_key_exists((int)$row['id'],$waterMap)?$waterMap[(int)$row['id']]:round($amount*max(0,(float)($row['drop_odds']??0)),2);$memberProfit=$win+$rebate+$offlineWater-$amount;$intercepted=(float)($map[(int)$row['id']]['intercepted']??0);$ratio=$amount>0?min(1,$intercepted/$amount):0;$numbers=preg_split('/[\s,，]+/u',trim((string)$row['number_text']),-1,PREG_SPLIT_NO_EMPTY)?:[];$row['bet_count']=max(1,count($numbers));$row['intercepted']=$intercepted;$row['requested_share']=(float)($map[(int)$row['id']]['requested']??0);$row['share_rate']=(float)($map[(int)$row['id']]['share_rate']??0);$row['offline_water']=$offlineWater;$row['house_profit']=-$memberProfit;$row['share_profit']=(-$memberProfit)*$ratio;}unset($row);return$rows;
    }
