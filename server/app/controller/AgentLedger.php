...
final class AgentLedger
{
    ...
    {
        if($issues===[])return[];$query=Db::name('bet_details')->alias('d')->join('bet_records r','r.id=d.bet_record_id')->join('site_users u','u.id=d.user_id')->leftJoin('user_stop_drops s','s.bet_detail_id=d.id')->where('d.site_id',(int)$session['site_id'])->whereIn('d.issue_no',$issues)->where('s.lottery',$lottery)->where('r.status','<>','refunded');OrganizationHierarchy::applyUserScope($query,$session,'d.user_id');$rows=$query->field('d.id,d.bet_record_id,r.status AS record_status,d.user_id,u.username,u.display_name,u.organization_id,u.interception_rate AS member_share_rate,d.issue_no,d.category,d.number_text,d.amount,d.win_amount,d.rebate,s.drop_odds')->select()->toArray();if($rows===[])return[];
        ...
        foreach(Db::name('organization_nodes')->where('site_id',(int)$session['site_id'])->field('id,level,parent_id')->select()->toArray() as $nodeRow){$nid=(int)$nodeRow['id'];$nodeLevels[$nid]=(string)$nodeRow['level'];$nodeParents[$nid]=(int)$nodeRow['parent_id'];}
        $viewerIsRoot=($nodeParents[$viewerOrg]??1)===0;
        // Settled records keep the per-node rate snapshot from the credit
        ...
        foreach($rows as &$row){$amount=(float)$row['amount'];$win=(float)$row['win_amount'];$rebate=(float)$row['rebate'];$memberProfit=$win+$rebate-$amount;$intercepted=(float)($map[(int)$row['id']]['intercepted']??0);
            // Occupation follows the reference model, same as the betting
            // report: the viewer's share is its edge rate applied to the
            // residual turnover arriving at its level, its share P/L carries
            // the member net result minus water on the occupied amount, and
            // offline water accrues on the book of the level directly below.
    ...
    private function ledgerRows(array $details,string $view,array $session): array
    {
        ...
        foreach($details as $row){$head=$monthly?(string)$row['issue_no']:($path?$agentName:(trim((string)($row['display_name']??''))!==''?(string)$row['display_name']:(string)$row['username']));$key=$head.'|'.(string)$row['category'];if(!isset($groups[$key]))$groups[$key]=['label'=>$head,'category'=>(string)($row['category']?:'未分类'),'bet_count'=>0,'amount'=>0.0,'rebate'=>0.0,'water'=>0.0,'win_amount'=>0.0,'profit'=>0.0];$groups[$key]['bet_count']+=(int)$row['bet_count'];$groups[$key]['amount']+=(float)$row['amount'];$groups[$key]['rebate']+=(float)$row['rebate'];$groups[$key]['water']+=(float)($row['water']??0);$groups[$key]['win_amount']+=(float)$row['win_amount'];$groups[$key]['profit']+=(float)($row['agent_profit']??0);}
