...
final class Organization
{
    ...
    {
        $prefix=['shareholder'=>'SH','director'=>'DIR','general_agent'=>'GA','agent'=>'AG'][$level]??'ORG';
    ...
    private function nodePayload(Request $request,array $site,?array $current=null,?array $forcedParent=null): array
    {
        ...
        $level=$forcedParent?(string)OrganizationHierarchy::nextLevel((string)$forcedParent['level']):(string)$request->post('level',$current['level']??'shareholder');
        if (!in_array($level,OrganizationHierarchy::LEVELS,true)) throw new \InvalidArgumentException('组织层级无效');
        if ($level==='shareholder'&&$parentId!==0) throw new \InvalidArgumentException('股东必须是站点根节点');
        if ($level!=='shareholder'&&(!$parent||OrganizationHierarchy::nextLevel((string)$parent['level'])!==$level)) throw new \InvalidArgumentException('上下级关系不符合股东、总监、总代理、代理的顺序');
    ...
    private function responseForSite(int $siteId,?int $parentId=null): array
    {
        ...
        foreach($accounts as &$account){unset($account['password']);$account['permissions']=OrganizationHierarchy::decodePermissions($account['permissions']??null);}unset($account);
        ...
        foreach($nodes as &$node){
            $node['permissions']=OrganizationHierarchy::decodePermissions($node['permissions']??null);$node['settings']=json_decode((string)($node['settings']??''),true)?:[];$node['level_label']=OrganizationHierarchy::LABELS[(string)$node['level']]??$node['level'];$node['next_level']=OrganizationHierarchy::nextLevel((string)$node['level']);$node['balance']=number_format((float)($node['balance']??0),2,'.','');
    ...
    public function profile(Request $request): \think\response\Json
    {
        ...
        $level=(string)($node['level']??'shareholder');return $this->reply(['site'=>['id'=>(int)$site['id'],'name'=>$site['name']],'organization'=>$node?['id'=>(int)$node['id'],'name'=>(string)($session['username']??$node['name']),'level'=>$level,'level_label'=>OrganizationHierarchy::LABELS[$level]??$level,'next_level'=>OrganizationHierarchy::nextLevel($level),'credit'=>OrganizationHierarchy::nodeCreditSummary((int)$node['id'])]:null,'permissions'=>(array)($session['permissions']??['*']),'username'=>(string)($session['username']??'')]);
    }
    ...
    private function saveProfitShare(Request $request,array $site,int $childId,?int $forcedParent=null): array
    {
        ...
        $parent=$parentId>0?Db::name('organization_nodes')->where('id',$parentId)->where('site_id',(int)$site['id'])->whereNull('deleted_at')->find():null; if($parentId>0&&!$parent) throw new \InvalidArgumentException('上级组织不存在'); if($parentId===0&&(string)$child['level']!=='shareholder') throw new \InvalidArgumentException('只有根股东可以直接归属平台');
