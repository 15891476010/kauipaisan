...
final class Organization
{
    ...
    private function responseForSite(int $siteId,?int $parentId=null): array
    {
        ...
        return ['nodes'=>$nodes,'members'=>$members,'accounts'=>$accounts,'boards'=>$this->activeBoards((int)$site['tenant_id']),'catalog'=>$this->catalog($siteId,$catalogLevel,$catalogParentPermissions,$catalogParentLevel),'site_max_share_rate'=>number_format($siteCap,4,'.','')];
    }
    ...
    public function adminUpdateNode(Request $request,int $id): \think\response\Json { $current=Db::name('organization_nodes')->where('id',$id)->whereNull('deleted_at')->find();if(!$current)throw new \InvalidArgumentException('组织不存在');$site=$this->site((int)$current['site_id']);$data=$this->nodePayload($request,$site,$current);$operator=$this->requestOperator($request);Db::transaction(function()use($id,$current,$data,$request,$operator):void{$delta=(float)$data['credit_limit']-(float)$current['credit_limit'];ScoreTransfer::organizationAllocation($current,$delta,$operator);Db::name('organization_nodes')->where('id',$id)->update($data);$updatedNode=array_merge($current,$data,['id'=>$id]);$account=Db::name('organization_accounts')->where('organization_id',$id)->whereNull('deleted_at')->order('id asc')->find();if($account){$accountData=$this->accountPayload($request,$updatedNode,$account);Db::name('organization_accounts')->where('id',(int)$account['id'])->update($accountData);}elseif(trim((string)$request->post('username',''))!==''&&trim((string)$request->post('password',''))!==''){ $accountData=$this->accountPayload($request,$updatedNode);unset($accountData['_initial_password']);$accountData['created_at']=$accountData['updated_at'];$this->createOrRestoreAccount($accountData);}});OrganizationHierarchy::rebuildBranch($id);return $this->reply(null,'组织和账号已更新'); }
    ...
    public function adminSetDirectorCreditShare(Request $request,int $id): \think\response\Json
    {
        ...
        $result=Db::transaction(function()use($id,$credit,$max,$children,$operator,$node):array{
            ...
            // 直比语义下总监的占成 = 100% − 全部直属下级占比之和（顶层吃残余）。
            $childrenShareSum=0.0;foreach($children as $child){$childrenShareSum+=(float)Db::name('organization_profit_shares')->where('child_organization_id',(int)$child['id'])->where('status',1)->value('share_rate');}
            $rootShareRate=max(0.0,100.0-$childrenShareSum);
            $rootShare=Db::name('organization_profit_shares')->where('child_organization_id',$id)->where('parent_organization_id',0)->find();$rootData=['tenant_id'=>(int)$node['tenant_id'],'site_id'=>(int)$node['site_id'],'parent_organization_id'=>0,'child_organization_id'=>$id,'max_share_rate'=>number_format($max,4,'.',''),'share_rate'=>number_format($rootShareRate,4,'.',''),'status'=>1,'updated_at'=>$now];if($rootShare)Db::name('organization_profit_shares')->where('id',(int)$rootShare['id'])->update($rootData);else{$rootData['created_at']=$now;Db::name('organization_profit_shares')->insert($rootData);}
    ...
    private function saveProfitShare(Request $request,array $site,int $childId,?int $forcedParent=null): array
    {
        ...
        // 直比占成：同一上级的兄弟节点占比之和不能超过 100%。
        $siblingSum=(float)Db::name('organization_profit_shares')->where('site_id',(int)$site['id'])->where('parent_organization_id',$parentId)->where('child_organization_id','<>',$childId)->where('status',1)->sum('share_rate');if($siblingSum+$rate>100.000001) throw new \InvalidArgumentException('同层占成合计将超过 100%（当前其他兄弟合计 '.number_format($siblingSum,4,'.','').'%），请先下调其他下级占成');
